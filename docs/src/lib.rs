...
let weights = iris_core::safetensors::load(weights_filename, &Device::Cpu).unwrap();
...
let weights = iris_core::safetensors::load_buffer(&mmap[..], &Device::Cpu).unwrap();
...
let mnist = iris_datasets::vision::Dataset {
    train_images,
    train_labels,
    test_images,
    test_labels,
