...
class Linear(Module):
    ...
    def __init__(
        ...
        self.weight = iris.ones((out_features, in_features), **factory_kwargs)
        if bias:
            self.bias = iris.zeros((out_features,), **factory_kwargs)
        else:
            self.bias = None
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if isinstance(self.weight, iris.QTensor):
            if len(dims) < 3:
