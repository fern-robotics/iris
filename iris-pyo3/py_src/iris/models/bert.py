...
from iris.nn import Module, Embedding, LayerNorm, Linear, ModuleList
from iris import Tensor
import iris
import iris.functional as F
...
class BertEmbeddings(Module):
    def __init__(self, config: Config) -> None:
        ...
        self.LayerNorm = LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.position_ids = iris.Tensor(list(range(config.max_position_embeddings))).reshape(
...
def masked_fill(on_false: float, mask: Tensor, on_true: float):
    ...
    on_false = iris.tensor(on_false).broadcast_as(shape)
