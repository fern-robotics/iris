...
_UNSIGNED_DTYPES = set([str(iris.u8), str(iris.u32)])
...
def assert_almost_equal(
    ...
    if str(actual.dtype) in _UNSIGNED_DTYPES or str(expected.dtype) in _UNSIGNED_DTYPES:
        actual = actual.to(iris.i64)
        expected = expected.to(iris.i64)

    diff = (actual - expected).abs()

    threshold = (expected.abs().to_dtype(iris.f32) * rtol + atol).to(expected)
