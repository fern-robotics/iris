# This example shows how the iris Python api can be used to replicate llama.cpp.
import sys
import iris
from iris.models.llama import QuantizedLlama
...
def main():
    ...
    for token_idx in range(500):
        last_token = tokens[-1]
        lt = iris.tensor([last_token]).unsqueeze(0)
        logits = model.forward(lt, len(tokens))
        # Greedy sampling for now
        # pr = iris.nn.softmax(logits, -1)
