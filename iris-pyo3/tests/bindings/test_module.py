import iris
from iris import Tensor, QTensor
from iris.nn import Module, Linear
...
def test_module_can_load_statedict():
    ...
    statedict = {
        "linear.weight": iris.ones((20, 10)),
        "linear.bias": iris.zeros((20,)),
        "t": Tensor(42.0),
    }
    a = A()
...
def test_module_can_load_quantized_tensors():
    class A(Module):
        def __init__(self):
            super().__init__()
            self.t = iris.randn((16, 256))
            self._quantizable_buffers.add("t")

    statedict = {
        "t": iris.ones((16, 256)).quantize("q4_0"),
    }
    a = A()
    a.load_state_dict(statedict)
