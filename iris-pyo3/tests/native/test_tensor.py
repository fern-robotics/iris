...
from iris.utils import cuda_is_available
from iris.testing import assert_equal
import pytest
...
def assert_bool(t: Tensor, expected: bool):
    assert t.shape == ()
    assert str(t.dtype) == str(iris.u8)
...
def test_tensor_equality_operations_can_broadcast():
    ...
    #  [1,1,0]
    #  [1,1,1]]
    mask_cond = iris.Tensor([0, 1, 2])
    mask = mask_cond < (mask_cond + 1).reshape((3, 1))
    assert mask.shape == (3, 3)
    assert_equal(mask, Tensor([[1, 0, 0], [1, 1, 0], [1, 1, 1]]).to_dtype(iris.u8))
...
def test_tensor_can_be_index_via_tensor():
    t = iris.Tensor([[1, 2, 1, 2], [3, 4, 3, 4], [5, 6, 5, 6]])
    indexed = t[iris.Tensor([0, 2])]
...
def test_tensor_can_be_cast_via_to():
    t = Tensor(42.0)
    assert str(t.dtype) == str(iris.f32)
    t_new_args = t.to(iris.f64)
    assert str(t_new_args.dtype) == str(iris.f64)
    t_new_kwargs = t.to(dtype=iris.f64)
    assert str(t_new_kwargs.dtype) == str(iris.f64)
    pytest.raises(TypeError, lambda: t.to("not a dtype"))
    pytest.raises(TypeError, lambda: t.to(dtype="not a dtype"))
    pytest.raises(TypeError, lambda: t.to(iris.f64, "not a dtype"))
    pytest.raises(TypeError, lambda: t.to())
    pytest.raises(ValueError, lambda: t.to(iris.f16, dtype=iris.f64))
    pytest.raises(ValueError, lambda: t.to(iris.f16, iris.f16))

    other = Tensor(42.0).to(iris.f64)
    t_new_other_args = t.to(other)
    assert str(t_new_other_args.dtype) == str(iris.f64)
    t_new_other_kwargs = t.to(other=other)
    assert str(t_new_other_kwargs.dtype) == str(iris.f64)
...
def test_tensor_can_be_moved_and_cast_via_to():
    ...
    assert t.device == "cpu"
    assert str(t.dtype) == str(iris.f32)
    t_new_args = t.to("cuda", iris.f64)
    assert t_new_args.device == "cuda"
    assert str(t_new_args.dtype) == str(iris.f64)
    t_new_kwargs = t.to(device="cuda", dtype=iris.f64)
    assert t_new_kwargs.device == "cuda"
    assert str(t_new_kwargs.dtype) == str(iris.f64)

    other = Tensor(42.0).to("cuda").to(iris.f64)
    t_new_other_args = t.to(other)
    assert t_new_other_args.device == "cuda"
    assert str(t_new_other_args.dtype) == str(iris.f64)
    t_new_other_kwargs = t.to(other=other)
