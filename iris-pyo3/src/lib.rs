...
use ::iris_core::{quantized::QTensor, DType, Device, Module, Tensor, WithDType};
...
fn actual_index(t: &Tensor, dim: usize, index: i64) -> ::iris_core::Result<usize> {
    let dim = t.dim(dim)?;
    if 0 <= index {
        let index = index as usize;
        if dim <= index {
            ::iris_core::bail!("index {index} is too large for tensor dimension {dim}")
        }
        Ok(index)
    } else {
        if (dim as i64) < -index {
            ::iris_core::bail!("index {index} is too low for tensor dimension {dim}")
        }
        Ok((dim as i64 + index) as usize)
    }
}

fn actual_dim(t: &Tensor, dim: i64) -> ::iris_core::Result<usize> {
    let rank = t.rank();
    if 0 <= dim {
        let dim = dim as usize;
        if rank <= dim {
            ::iris_core::bail!("dimension index {dim} is too large for tensor rank {rank}")
        }
        Ok(dim)
    } else {
        if (rank as i64) < -dim {
            ::iris_core::bail!("dimension index {dim} is too low for tensor rank {rank}")
        }
        Ok((rank as i64 + dim) as usize)
    }
}

// TODO: Something similar to this should probably be a part of iris core.
trait MapDType {
    type Output;
...
impl PyTensor {
    ...
    /// Converts iris's tensor to pytorch's tensor
    /// &RETURNS&: torch.Tensor
    fn to_torch(&self, py: Python<'_>) -> PyResult<Py<PyAny>> {
        let iris_values = self.values(py)?;
    ...
    fn __richcmp__(&self, rhs: &Bound<PyAny>, op: CompareOp) -> PyResult<Self> {
        ...
        if let Ok(rhs) = rhs.extract::<PyTensor>() {
            if self.0.shape() == rhs.0.shape() {
                ...
                // We broadcast manually here because `iris.cmp` does not support automatic broadcasting
...
impl PyQTensor {
    ...
    fn matmul_t(&self, lhs: &PyTensor) -> PyResult<PyTensor> {
        let qmatmul = ::iris_core::quantized::QMatMul::from_arc(self.0.clone()).map_err(wrap_err)?;
...
fn load_safetensors(path: &str, py: Python<'_>) -> PyResult<Py<PyAny>> {
    let res = ::iris_core::safetensors::load(path, &Device::Cpu).map_err(wrap_err)?;
    let res = res
        .into_iter()
...
fn save_safetensors(
    ...
    ::iris_core::safetensors::save(&tensors, path).map_err(wrap_err)
}
...
fn load_ggml<'py>(
    ...
    let device = device.unwrap_or(PyDevice::Cpu).as_device()?;
    let ggml =
        ::iris_core::quantized::ggml_file::Content::read(&mut file, &device).map_err(wrap_err)?;
    let tensors = ggml
        .tensors
        .into_iter()
...
fn load_gguf<'py>(
    ...
    let device = device.unwrap_or(PyDevice::Cpu).as_device()?;
    use ::iris_core::quantized::gguf_file;
...
fn cuda_is_available() -> bool {
    ::iris_core::utils::cuda_is_available()
}

#[pyfunction]
/// Returns true if iris was compiled with 'accelerate' support.
/// &RETURNS&: bool
fn has_accelerate() -> bool {
    ::iris_core::utils::has_accelerate()
}

#[pyfunction]
/// Returns true if iris was compiled with MKL support.
/// &RETURNS&: bool
fn has_mkl() -> bool {
    ::iris_core::utils::has_mkl()
}

#[pyfunction]
/// Returns the number of threads used by the iris.
/// &RETURNS&: int
fn get_num_threads() -> usize {
    ::iris_core::utils::get_num_threads()
}

fn iris_utils(_py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
...
fn softmax(tensor: PyTensor, dim: i64) -> PyResult<PyTensor> {
    ...
    let sm = iris_nn::ops::softmax(&tensor.0, dim).map_err(wrap_err)?;
    Ok(PyTensor(sm))
}
...
fn silu(tensor: PyTensor) -> PyResult<PyTensor> {
    let s = iris_nn::ops::silu(&tensor.0).map_err(wrap_err)?;
    Ok(PyTensor(s))
}
...
fn iris_functional_m(_py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
...
#[cfg(feature = "onnx")]
fn iris_onnx_m(_py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
...
#[pymodule]
fn iris(py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
    let utils = PyModule::new(py, "utils")?;
    iris_utils(py, &utils)?;
    m.add_submodule(&utils)?;
    let nn = PyModule::new(py, "functional")?;
    iris_functional_m(py, &nn)?;
    m.add_submodule(&nn)?;
    #[cfg(feature = "onnx")]
    {
