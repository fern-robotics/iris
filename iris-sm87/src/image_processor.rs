...
use iris_core::{bail, DType, Device, Result, Tensor};
use iris_transformers::models::llava::{
...
impl ImageProcessor {
    pub fn from_pretrained(clip_id: &str) -> Result<Self> {
        let api = Api::new().map_err(|e| iris_core::Error::Msg(e.to_string()))?;
        let api = api.model(clip_id.to_string());
        let config_filename = api
            .get("preprocessor_config.json")
            .map_err(|e| iris_core::Error::Msg(e.to_string()))?;
        let image_processor =
            serde_json::from_slice(&std::fs::read(config_filename).map_err(iris_core::Error::Io)?)
                .map_err(|e| iris_core::Error::Msg(e.to_string()))?;
        Ok(image_processor)
    }
