...
impl FlashAttn {
    ...
    >(
        ...
        if !(head_size_og == 512
            ...
            || head_size_og == 64)
        {
            iris_core::bail!("only supports head dimension 64, 128, 256 and 512 (got {head_size_og})")
        }
        if head_size_og % 8 != 0 {
...
impl iris_core::CustomOp3 for FlashAttn {
    ...
    fn cpu_fwd(
        ...
        iris_core::bail!("no cpu support for flash-attn-v3")
    }
    ...
    fn cuda_fwd(
        ...
        match q.dtype() {
            ...
            dt => iris_core::bail!("flash-attn-v3 is only supported for f16/bf16 ({dt:?})"),
        }
    }
}
