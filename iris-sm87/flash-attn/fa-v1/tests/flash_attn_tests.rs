use anyhow::Result;
use iris_core::{DType, Device, IndexOp, Tensor, D};
...
fn fa_acausal(q: &Tensor, k: &Tensor, v: &Tensor, softmax_scale: f32) -> Result<Tensor> {
    ...
    let att = iris_nn::ops::softmax(&att, D::Minus1)?;
...
fn flash_attn_acausal() -> Result<()> {
    ...
    let ys2 = {
        ...
        iris_flash_attn::flash_attn(&q, &k, &v, 0.5, false)?.transpose(1, 2)?
    };
...
fn flash_attn_acausal_softcap() -> Result<()> {
    ...
    let ys2 = {
        ...
        let k = k.transpose(1, 2)?;
        let v = v.transpose(1, 2)?;
        iris_flash_attn::flash_attn_alibi_windowed_softcap(
            &q,
            &k,
            &v,
            None,            //  alibi_slopes //
...
fn flash_attn_varlen_paged_mm_prefix_windowed() -> Result<()> {
    ...
    let ys_causal = iris_flash_attn::flash_attn_varlen_paged_windowed(
        &q,
        &k_paged,
        &v_paged,
        &seqlens,
    ...
    let ys = iris_flash_attn::flash_attn_varlen_paged_windowed(
        &q,
        &k_paged,
        &v_paged,
        &seqlens,
...
fn flash_attn_varlen() -> Result<()> {
    ...
    let ys = {
        ...
        iris_flash_attn::flash_attn_varlen(
