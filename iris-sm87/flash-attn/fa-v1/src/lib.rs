...
use iris_core::cuda_backend::cudarc::driver::{DevicePtr, DevicePtrMut};
use iris_core::{CpuStorage, DType, Layout, Result, Shape, Tensor};
use half::{bf16, f16};
...
impl FlashAttn {
    ...
    >(
        &self,
        q: &iris_core::CudaStorage,
        q_l: &Layout,
        k: &iris_core::CudaStorage,
        k_l: &Layout,
        v: &iris_core::CudaStorage,
        v_l: &Layout,
        is_bf16: bool,
        ...
        if q_stride[q_rank - 1] != 1 {
            iris_core::bail!("the last dim of q must be contiguous {q_stride:?}")
        }
        if k_stride[k_rank - 1] != 1 {
            iris_core::bail!("the last dim of k must be contiguous {k_stride:?}")
        }
        if v_stride[v_rank - 1] != 1 {
            iris_core::bail!("the last dim of v must be contiguous {v_stride:?}")
        }
        ...
        if expected_kv != k_l.shape().dims4()? {
            iris_core::bail!("shape mismatch q {:?} and k {:?}", q_l.shape(), k_l.shape())
        }
        if expected_kv != v_l.shape().dims4()? {
            iris_core::bail!("shape mismatch q {:?} and v {:?}", q_l.shape(), v_l.shape())
        }
        if head_size_og > 512 {
            iris_core::bail!("only supports head dimension at most 512 (got {head_size_og})")
        }
        if head_size_og % 8 != 0 {
            // TODO: Handle head sizes that are not a multiple of 8 via some padding.
            iris_core::bail!("only supports head sizes that are a multiple of 8 (got {head_size_og})")
        }
        if num_heads % num_heads_k != 0 {
            iris_core::bail!("number of k/v heads {num_heads_k} must divide number of heads in query {num_heads}")
        }
        ...
        let alibi_slopes_ptr = if let Some(alibi_slopes) = &self.alibi_slopes {
            ...
            let alibi_slopes = match &*alibi_slopes {
                ...
                _ => iris_core::bail!("alibi_slopes must be a cuda tensor"),
            };
        ...
        let dst = iris_core::CudaStorage::wrap_cuda_slice(dst, dev.clone());
        Ok((dst, out_shape))
    }
}

impl iris_core::CustomOp3 for FlashAttn {
    ...
    fn cpu_fwd(
        ...
        iris_core::bail!("no cpu support for flash-attn")
    }
    ...
    fn cuda_fwd(
        ...
        match q.dtype() {
            iris_core::DType::F16 => self.cuda_fwd_t::<f16>(q, q_l, k, k_l, v, v_l, false),
            iris_core::DType::BF16 => self.cuda_fwd_t::<bf16>(q, q_l, k, k_l, v, v_l, true),
            dt => iris_core::bail!("flash-attn is only supported for f16/bf16 ({dt:?})"),
        }
    }
}
...
impl FlashAttnVarLen {
    ...
    >(
        ...
        let seqlens_q = match &*seqlens_q {
            iris_core::Storage::Cuda(c) => c.as_cuda_slice::<u32>()?, // Should be i32!
            _ => iris_core::bail!("seqlens_q must be a cuda tensor"),
        };
        let seqlens_q = match seqlens_q_layout.contiguous_offsets() {
            Some((o1, o2)) => seqlens_q.slice(o1..o2),
            None => iris_core::bail!("seqlens_q has to be contiguous"),
        };

        let (seqlens_k, seqlens_k_layout) = self.seqlens_k.storage_and_layout();
        let seqlens_k = match &*seqlens_k {
            iris_core::Storage::Cuda(c) => c.as_cuda_slice::<u32>()?, // Should be i32!
            _ => iris_core::bail!("seqlens_k must be a cuda tensor"),
        };
        let seqlens_k = match seqlens_k_layout.contiguous_offsets() {
            Some((o1, o2)) => seqlens_k.slice(o1..o2),
            None => iris_core::bail!("seqlens_k has to be contiguous"),
        };
        ...
        let block_table = if let Some(block_table) = self.block_table.as_ref() {
            ...
            match &*block_table_storage {
                iris_core::Storage::Cuda(_) => {}
                _ => iris_core::bail!("block_table must be a cuda tensor"),
            }
            let block_table_stride = block_table_layout.shape().dims2()?.1;
            if block_table_layout.stride().last().copied() != Some(1) {
                iris_core::bail!("block_table last dimension must be contiguous")
            }
            Some((
                block_table_storage,
        ...
        let (num_heads_k, page_block_size) = if paged {
            ...
            if expected_v != v_l.shape().dims4()? {
                iris_core::bail!("shape mismatch k {:?} and v {:?}", k_l.shape(), v_l.shape())
            }
            if k_head_size != head_size_og {
                iris_core::bail!("shape mismatch q {:?} and k {:?}", q_l.shape(), k_l.shape())
            }
            let Some(page_block_size_arg) = self.page_block_size else {
                iris_core::bail!("paged flash-attn requires page_block_size")
            };
            ...
            if head_size_og > 512 {
                iris_core::bail!("paged flash-attn supports head sizes up to 512 (got {head_size_og})")
            }
        ...
        if nseqlens_q < 2 {
            iris_core::bail!("seqlens_q should have a len >= 2 {nseqlens_q}")
        }
        let nseqlens_k = seqlens_k_layout.shape().dims1()?;
        if nseqlens_k != nseqlens_q {
            iris_core::bail!("seqlens_q and seqlens_k should have the same number of elements {nseqlens_q} <> {nseqlens_k}")
        }
        ...
        let mm_prefix_ranges = if let Some(mm_prefix_ranges) = self.mm_prefix_ranges.as_ref() {
            ...
            match &*storage {
                iris_core::Storage::Cuda(_) => {}
                _ => iris_core::bail!("mm_prefix_ranges must be a cuda tensor"),
            }
            ...
            if layout.stride().last().copied() != Some(1) {
                iris_core::bail!("mm_prefix_ranges last dimension must be contiguous")
            }
            Some((
                storage,
                layout.start_offset(),
        ...
        unsafe {
            ...
            let (block_table_ptr, block_table_batch_stride) =
                if let Some((block_table, offset, stride)) = block_table.as_ref() {
                    match (&**block_table, self.block_table.as_ref().unwrap().dtype()) {
                        (iris_core::Storage::Cuda(block_table), DType::U32) => {
                        ...
                        (iris_core::Storage::Cuda(block_table), DType::I32) => {
                        ...
                        (_, dtype) => {
                            iris_core::bail!("block_table must be u32 or i32, got {dtype:?}")
                        }
                    }
                } else {
                    (std::ptr::null(), 0)
            ...
            let (mm_prefix_ranges_ptr, mm_prefix_range_batch_stride, max_mm_prefix_ranges) =
                if let Some((storage, offset, stride, max_ranges)) = mm_prefix_ranges.as_ref() {
                    match &**storage {
                        iris_core::Storage::Cuda(mm_prefix_ranges) => {
...
impl iris_core::CustomOp3 for FlashAttnVarLen {
    fn name(&self) -> &'static str {
        "flash-attn-varlen"
    }
