...
use iris_core::quantized::GgmlType;
use iris_core::{CpuStorage, Device, Layout, Module, Result, Shape, Tensor, D};
...
impl iris_core::CustomOp1 for Im2Col {
...
impl Benchmark for QMatMul {
    type PreProcessData = (iris_core::quantized::QMatMul, Tensor);
    type RunResult = Tensor;
    fn preprocess() -> Result<Self::PreProcessData> {
        let zeros = vec![iris_core::quantized::k_quants::BlockQ4_0::zeros(); 4096 * 11008 / 32];
        let mm = iris_core::quantized::QTensor::new(
            iris_core::quantized::QStorage::Cpu(Box::new(zeros)),
            (4096, 11008),
        )?;
        let mm = iris_core::quantized::QMatMul::from_qtensor(mm)?;
...
impl Benchmark for Softmax {
    ...
    fn run_one(d: &Self::PreProcessData) -> Result<Self::RunResult> {
        iris_nn::ops::softmax(d, D::Minus1)
    }
...
impl Benchmark for SoftmaxLastDim {
    ...
    fn run_one(d: &Self::PreProcessData) -> Result<Self::RunResult> {
        iris_nn::ops::softmax_last_dim(d)
    }
