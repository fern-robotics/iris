//! Rotary Embeddings
//!
use iris_core::{CpuStorage, Layout, Result, Shape, Tensor, D};
use rayon::prelude::*;
...
impl iris_core::CustomOp3 for RotaryEmbI {
    ...
    fn cpu_fwd(
        ...
        fn inner<T: iris_core::WithDType + num_traits::Float>(
            src: &[T],
            l_src: &Layout,
            cos: &[T],
            l_cos: &Layout,
            ...
            let src = match l_src.contiguous_offsets() {
                None => iris_core::bail!("input src has to be contiguous"),
                Some((o1, o2)) => &src[o1..o2],
            };
            let cos = match l_cos.contiguous_offsets() {
                None => iris_core::bail!("input cos has to be contiguous"),
                Some((o1, o2)) => &cos[o1..o2],
            };
            let sin = match l_sin.contiguous_offsets() {
                None => iris_core::bail!("input sin has to be contiguous"),
    ...
    fn cuda_fwd(
        ...
        use iris_core::cuda_backend::{kernel_name, kernels, WrapErr};
        ...
        fn inner<T: DeviceRepr + WithDType>(
            ...
            let src = match l_src.contiguous_offsets() {
                None => iris_core::bail!("src input has to be contiguous"),
                Some((o1, o2)) => src.slice(o1..o2),
            };
            let cos = match l_cos.contiguous_offsets() {
                None => iris_core::bail!("cos input has to be contiguous"),
                Some((o1, o2)) => cos.slice(o1..o2),
            };
            let sin = match l_sin.contiguous_offsets() {
                None => iris_core::bail!("sin input has to be contiguous"),
            ...
            builder.arg(&sin);
            builder.arg(&dst);
            iris_core::builder_arg!(builder, (b * h) as u32, (t * d) as u32, stride_b);
            // SAFETY: ffi.
        ...
        use iris_core::backend::BackendStorage;
        use iris_core::cuda_backend::CudaStorageSlice::{BF16, F16, F32, F64};
        let dev = s1.device();
    ...
    fn metal_fwd(
        &self,
        src: &iris_core::MetalStorage,
        l_src: &Layout,
        cos: &iris_core::MetalStorage,
        l_cos: &Layout,
        sin: &iris_core::MetalStorage,
        l_sin: &Layout,
        ...
        let name = match src.dtype() {
            iris_core::DType::F32 => "rope_i_f32",
            iris_core::DType::F16 => "rope_i_f16",
            iris_core::DType::BF16 => "rope_i_bf16",
            dtype => iris_core::bail!("rope-i is not implemented for {dtype:?}"),
        };
        let (b, h, t, d) = l_src.shape().dims4()?;
        let stride_b = if l_cos.dims().len() == 3 && l_sin.dims().len() == 3 {
            h * t * d
        ...
        let output = device
            ...
            .with_label("rope_i")
            .build()?;
        iris_metal_kernels::call_rope_i(
            device.metal_device(),
            &encoder,
            kernels,
            name,
        ...
        let out = iris_core::MetalStorage::new(output, device.clone(), el, src.dtype());
...
fn rope_check_cs(cs: &Tensor, b_sz: usize) -> Result<(usize, usize)> {
    match *cs.dims() {
        ...
        [b, t, d] => {
            if b != b_sz {
                iris_core::bail!("inconsistent batch size in rope {b_sz} {cs:?}",)
            }
            Ok((t, d))
        }
        _ => iris_core::bail!("cos/sin has to be 2D or 3D in rope {b_sz} {cs:?}"),
    }
}
...
pub fn rope_i(xs: &Tensor, cos: &Tensor, sin: &Tensor) -> Result<Tensor> {
    ...
    if !xs.is_contiguous() {
        iris_core::bail!("xs has to be contiguous in rope")
    }
    if !cos.is_contiguous() {
        iris_core::bail!("cos has to be contiguous in rope")
    }
    if !sin.is_contiguous() {
        iris_core::bail!("sin has to be contiguous in rope")
    }
...
impl iris_core::CustomOp3 for RotaryEmb {
    fn name(&self) -> &'static str {
        "rotary-emb"
    }
    ...
    fn cuda_fwd(
        ...
        fn inner<T: DeviceRepr + WithDType>(
            ...
            builder.arg(&sin);
            builder.arg(&dst);
            iris_core::builder_arg!(builder, (b * h) as u32, (t * d) as u32, d as u32, stride_b);
            // SAFETY: ffi.
            unsafe { builder.launch(cfg) }.w()?;
            Ok(dst)
        }
    ...
    fn metal_fwd(
        ...
        let name = match src.dtype() {
            iris_core::DType::F32 => "rope_f32",
            iris_core::DType::F16 => "rope_f16",
            iris_core::DType::BF16 => "rope_bf16",
            dtype => iris_core::bail!("rope is not implemented for {dtype:?}"),
        };
        let (b, h, t, d) = l_src.shape().dims4()?;
        let stride_b = if l_cos.dims().len() == 3 && l_sin.dims().len() == 3 {
            h * t * d
...
impl iris_core::CustomOp3 for RotaryEmbThd {
    fn name(&self) -> &'static str {
        "rotary-emb"
    }
    ...
    fn cuda_fwd(
        ...
        fn inner<T: DeviceRepr + WithDType>(
            ...
            builder.arg(&sin);
            builder.arg(&dst);
            iris_core::builder_arg!(builder, b as u32, t as u32, h as u32, d as u32, stride_b);
            // SAFETY: ffi.
            unsafe { builder.launch(cfg) }.w()?;
            Ok(dst)
        }
    ...
    fn metal_fwd(
        ...
        let name = match src.dtype() {
            iris_core::DType::F32 => "rope_thd_f32",
            iris_core::DType::F16 => "rope_thd_f16",
            iris_core::DType::BF16 => "rope_thd_bf16",
            dtype => iris_core::bail!("rope_thd is not implemented for {dtype:?}"),
        };
        let (b, t, h, d) = l_src.shape().dims4()?;
        let stride_b = if l_cos.dims().len() == 3 && l_sin.dims().len() == 3 {
            h * t * d
        ...
        let output = device
            ...
            .with_label("rope_thd")
            .build()?;
        iris_metal_kernels::call_rope_thd(
            device.metal_device(),
            &encoder,
            kernels,
            name,
