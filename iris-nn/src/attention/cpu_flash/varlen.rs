...
use iris_core::{DType, Device, Result, Storage, Tensor};
use half::f16;
use rayon::prelude::*;
...
pub fn flash_attn_varlen_cpu(
    ...
    if !q.device().is_cpu() {
        iris_core::bail!("flash_attn_varlen_cpu is CPU only");
    }
    ...
    if dt != DType::F32 && dt != DType::F16 {
        iris_core::bail!("flash_attn_varlen_cpu supports f32 or f16 only");
    }
    ...
    if dk != d || dv != d {
        iris_core::bail!("head dim mismatch");
    }

    if hq % hk != 0 {
        iris_core::bail!("Invalid GQA: Hq={} not divisible by Hk={}", hq, hk);
    }
    if hq % hv != 0 {
        iris_core::bail!("Invalid GQA: Hq={} not divisible by Hv={}", hq, hv);
    }
    let rk = hq / hk;
    let rv = hq / hv;
    ...
    if seqlens_k_vec.len() != bsz {
        iris_core::bail!("seqlens_q and seqlens_k batch mismatch");
    }
    ...
    if causal {
        for i in 0..bsz {
            ...
            if lk < lq {
                iris_core::bail!("causal requires lk>=lq, got lk={lk} lq={lq} at batch {i}");
            }
        }
    }
    ...
    if cumsum_q[bsz] != total_q || cumsum_k[bsz] != total_k {
        iris_core::bail!("total_q/total_k mismatch with seqlens");
    }

    let slopes: Option<Vec<f32>> = if let Some(s) = alibi_slopes {
        let v = s.to_vec1::<f32>()?;
        if v.len() != hq {
            iris_core::bail!("alibi_slopes len {} != Hq {}", v.len(), hq);
        }
        Some(v)
    } else {
        None
