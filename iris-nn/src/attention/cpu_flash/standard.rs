...
use iris_core::{Device, Result, Storage, Tensor, WithDType};
use rayon::prelude::*;
use std::f32;
    ...
    T: WithDType,
{
    ...
    let q_data: &[T] = if let Storage::Cpu(cpu) = &*q_guard {
        ...
        return Err(iris_core::Error::Msg("Expected CPU storage for q".into()));
    };
    ...
    let k_data: &[T] = if let Storage::Cpu(cpu) = &*k_guard {
        ...
        return Err(iris_core::Error::Msg("Expected CPU storage for k".into()));
    };
    ...
    let v_data: &[T] = if let Storage::Cpu(cpu) = &*v_guard {
        ...
        return Err(iris_core::Error::Msg("Expected CPU storage for v".into()));
    };
    ...
    let mask_data: Option<&[T]> = if let Some((guard, layout)) = &mask_guard {
        if let Storage::Cpu(cpu) = &**guard {
            ...
            return Err(iris_core::Error::Msg("Expected CPU storage for mask".into()));
        }
    } else {
        None
    };
