...
use iris_core::{DType, Result, Tensor, Var};
...
impl BatchNorm {
    fn check_validity(&self, num_features: usize) -> Result<()> {
        if self.eps < 0. {
            iris_core::bail!("batch-norm eps cannot be negative {}", self.eps)
        }
...
pub fn batch_norm<C: Into<BatchNormConfig>>(
    ...
    if config.eps < 0. {
        iris_core::bail!("batch-norm eps cannot be negative {}", config.eps)
    }
