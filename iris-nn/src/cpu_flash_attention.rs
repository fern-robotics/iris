...
//!
//! **Deprecated:** Use `iris_nn::attention::{flash_attn, AttnMask}` instead.

use iris_core::{Result, Tensor, WithDType};

/// Deprecated: use `iris_nn::attention::flash_attn` with `AttnMask` instead.
///
/// This shim routes through the new dispatcher which handles both B=1 and B>1.
#[deprecated(
    since = "0.9.2",
    note = "Use `iris_nn::attention::{flash_attn, AttnMask}` instead"
)]
