...
//!
use iris_core::{DType, Device, Result, Shape, Tensor, Var};
...
impl VarMap {
    ...
    pub fn load<P: AsRef<std::path::Path>>(&mut self, path: P) -> Result<()> {
        ...
        let data = unsafe { iris_core::safetensors::MmapedSafetensors::new(path)? };
        let mut tensor_data = self.data.lock().unwrap();
        for (name, var) in tensor_data.iter_mut() {
            let data = data.load(name, var.device())?;
            if let Err(err) = var.set(&data) {
                iris_core::bail!("error setting {name} using data from {path:?}: {err}",)
            }
        }
        Ok(())
    }
    ...
    pub fn set_one<K: AsRef<str>, V: AsRef<Tensor>>(&mut self, name: K, value: V) -> Result<()> {
        ...
        match tensor_data.get(name) {
            None => iris_core::bail!("cannot find {name} in VarMap"),
            Some(var) => {
                if let Err(err) = var.set(value.as_ref()) {
                    iris_core::bail!("error setting {name}: {err}",)
                }
            }
        }
        Ok(())
    ...
    pub fn get<S: Into<Shape>>(
        ...
        if let Some(tensor) = tensor_data.get(path) {
            ...
            if &shape != tensor_shape {
                iris_core::bail!("shape mismatch on {path}: {shape:?} <> {tensor_shape:?}")
            }
