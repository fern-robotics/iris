...
use crate::VarMap;
use iris_core::{safetensors::Load, DType, Device, Error, Result, Shape, Tensor};
...
impl SimpleBackend for HashMap<String, Tensor> {
    fn get(
        ...
        let tensor = self
            ...
            .clone();
        if tensor.shape() != &s {
            Err(iris_core::Error::UnexpectedShape {
...
impl SimpleBackend for VarMap {
    ...
    fn get_unchecked(&self, _name: &str, _dtype: DType, _dev: &Device) -> Result<Tensor> {
        iris_core::bail!("`get_unchecked` does not make sense for `VarMap`, use `get`.");
    }
...
impl SimpleBackend for iris_core::npy::NpzTensors {
    fn get(
        &self,
        s: Shape,
        path: &str,
...
impl SimpleBackend for iris_core::pickle::PthTensors {
    fn get(
        &self,
        s: Shape,
        path: &str,
...
impl SimpleBackend for iris_core::safetensors::MmapedSafetensors {
    fn get(
        &self,
        s: Shape,
        name: &str,
...
impl SimpleBackend for iris_core::safetensors::BufferedSafetensors {
    fn get(
        &self,
        s: Shape,
        name: &str,
...
impl SimpleBackend for iris_core::safetensors::SliceSafetensors<'_> {
    fn get(
        &self,
        s: Shape,
        name: &str,
...
impl<'a> VarBuilder<'a> {
    ...
    pub unsafe fn from_mmaped_safetensors<P: AsRef<std::path::Path>>(
        ...
        let tensors = iris_core::safetensors::MmapedSafetensors::multi(paths)?;
    ...
    pub fn from_buffered_safetensors(data: Vec<u8>, dtype: DType, dev: &Device) -> Result<Self> {
        let tensors = iris_core::safetensors::BufferedSafetensors::new(data)?;
        Ok(Self::from_backend(Box::new(tensors), dtype, dev.clone()))
    }
    ...
    pub fn from_slice_safetensors(data: &'a [u8], dtype: DType, dev: &Device) -> Result<Self> {
        let tensors = iris_core::safetensors::SliceSafetensors::new(data)?;
        Ok(Self::from_backend(Box::new(tensors), dtype, dev.clone()))
    }
    ...
    pub fn from_npz<P: AsRef<std::path::Path>>(p: P, dtype: DType, dev: &Device) -> Result<Self> {
        let npz = iris_core::npy::NpzTensors::new(p)?;
    ...
    pub fn from_pth<P: AsRef<std::path::Path>>(p: P, dtype: DType, dev: &Device) -> Result<Self> {
        let pth = iris_core::pickle::PthTensors::new(p, None)?;
    ...
    pub fn from_pth_with_state<P: AsRef<std::path::Path>>(
        ...
        let pth = iris_core::pickle::PthTensors::new(p, Some(state_key))?;
        Ok(Self::from_backend(Box::new(pth), dtype, dev.clone()))
    }
    ...
    ///
    /// ```rust
    /// use iris_core::{Tensor, DType, Device};
    ///
    ...
    /// .into_iter()
    /// .collect();
    /// let vb = iris_nn::VarBuilder::from_tensors(tensors, DType::F32, &Device::Cpu);
...
pub struct ShardedSafeTensors(iris_core::safetensors::MmapedSafetensors);
...
impl Backend for ShardedSafeTensors {
    ...
    fn get(
        ...
        let iterator = if dim == 0 {
            ...
            iris_core::bail!("Get sharded on dimensions != 0 or 1")
        };
    ...
    fn get_unchecked(&self, _name: &str, _dtype: DType, _dev: &Device) -> Result<Tensor> {
        iris_core::bail!("`get_unchecked` does not make sense for `ShardedSafeTensors`, use `get`.");
    }
