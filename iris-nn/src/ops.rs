...
use iris_core::{CpuStorage, DType, Layout, Module, Result, Shape, Tensor, D};
use rayon::prelude::*;
...
///
/// ```rust
/// use iris_core::{Tensor, Device, test_utils::to_vec2_round};
/// let a = Tensor::new(&[[0f32, 1., 0., 1.], [-2., 2., 3., -3.]], &Device::Cpu)?;
/// let a = iris_nn::ops::softmax(&a, 1)?;
/// assert_eq!(
///     to_vec2_round(&a, 4)?,
///     &[
...
/// # Ok::<(), iris_core::Error>(())
/// ```
pub fn softmax<D: iris_core::shape::Dim>(xs: &Tensor, dim: D) -> Result<Tensor> {
...
pub fn log_softmax<D: iris_core::shape::Dim>(xs: &Tensor, d: D) -> Result<Tensor> {
...
impl iris_core::CustomOp1 for Sigmoid {
    fn name(&self) -> &'static str {
        "sigmoid"
    }

    fn cpu_fwd(&self, storage: &CpuStorage, layout: &Layout) -> Result<(CpuStorage, Shape)> {
        use iris_core::backend::BackendStorage;

        fn fwd<T: num_traits::Float>(v: T) -> T {
            (v.neg().exp() + T::one()).recip()
        }

        // FIXME: using `iris_core::map_dtype` causes compilation errors.
        let storage = match storage {
            CpuStorage::BF16(slice) => {
                CpuStorage::BF16(iris_core::cpu_backend::unary_map(slice, layout, fwd))
            }
            CpuStorage::F16(slice) => {
                CpuStorage::F16(iris_core::cpu_backend::unary_map(slice, layout, fwd))
            }
            CpuStorage::F32(slice) => {
                CpuStorage::F32(iris_core::cpu_backend::unary_map(slice, layout, fwd))
            }
            CpuStorage::F64(slice) => {
                CpuStorage::F64(iris_core::cpu_backend::unary_map(slice, layout, fwd))
            }
            _ => Err(iris_core::Error::UnsupportedDTypeForOp(
                storage.dtype(),
                self.name(),
            ))?,
        };
    ...
    fn cuda_fwd(
        &self,
        storage: &iris_core::CudaStorage,
        layout: &Layout,
    ) -> Result<(iris_core::CudaStorage, Shape)> {
        use iris_core::backend::BackendStorage;
        use iris_core::cuda_backend::cudarc::driver::{
            CudaSlice, DeviceRepr, LaunchConfig, PushKernelArg, ValidAsZeroBits,
        };
        use iris_core::cuda_backend::SlicePtrOrNull;
        use iris_core::cuda_backend::{kernel_name, kernels, Map1, WrapErr};
        use iris_core::{CudaDevice, WithDType};
        ...
        impl Map1 for S {
            fn f<T: DeviceRepr + WithDType + ValidAsZeroBits>(
                ...
                iris_core::builder_arg!(builder, el_count, dims.len());
    ...
    fn metal_fwd(
        &self,
        storage: &iris_core::MetalStorage,
        layout: &Layout,
    ) -> Result<(iris_core::MetalStorage, Shape)> {
        use iris_core::backend::BackendStorage;
        use iris_core::MetalError;
        ...
        let src = iris_metal_kernels::BufferOffset {
            buffer: storage.buffer(),
            offset_in_bytes: layout.start_offset() * storage.dtype().size_in_bytes(),
        };

        if layout.is_contiguous() {
            use iris_metal_kernels::unary::contiguous;
            let kernel_name = match dtype {
                DType::F16 => contiguous::sigmoid::HALF,
                DType::F32 => contiguous::sigmoid::FLOAT,
                DType::BF16 => contiguous::sigmoid::BFLOAT,
                dtype => {
                    iris_core::bail!("Metal contiguous unary sigmoid {dtype:?} not implemented")
                }
            };
            iris_metal_kernels::call_unary_contiguous(
                device.metal_device(),
                &encoder,
                device.kernels(),
                kernel_name,
            ...
            use iris_metal_kernels::unary::strided;
            let kernel_name = match dtype {
                DType::F16 => strided::sigmoid::HALF,
                DType::F32 => strided::sigmoid::FLOAT,
                DType::BF16 => strided::sigmoid::BFLOAT,
                dtype => {
                    iris_core::bail!("Metal strided unary sigmoid {dtype:?} not implemented")
                }
            };
            let dst = iris_metal_kernels::BufferOffset::zero_offset(&buffer);
            iris_metal_kernels::call_unary_strided(
                device.metal_device(),
                &encoder,
                device.kernels(),
                kernel_name,
        ...
        let new_storage = iris_core::MetalStorage::new(buffer, device.clone(), el_count, dtype);
...
pub fn dropout(xs: &Tensor, drop_p: f32) -> Result<Tensor> {
    ...
    if !(0. ..1.).contains(&drop_p) {
        iris_core::bail!("dropout probability has to be in [0, 1), got {drop_p}")
    }
...
impl iris_core::ModuleT for Dropout {
...
impl iris_core::CustomOp1 for SoftmaxLastDim {
    fn name(&self) -> &'static str {
        "softmax-last-dim"
    }

    fn cpu_fwd(&self, storage: &CpuStorage, layout: &Layout) -> Result<(CpuStorage, Shape)> {
        fn softmax<T: iris_core::WithDType + num_traits::Float>(
            src: &[T],
            layout: &Layout,
        ) -> Result<(CpuStorage, Shape)> {
            let src = match layout.contiguous_offsets() {
                None => iris_core::bail!("input has to be contiguous"),
            ...
            let storage = iris_core::WithDType::to_cpu_storage_owned(dst);
        ...
        match storage {
            ...
            _ => iris_core::bail!("unsupported dtype for softmax {:?}", storage),
        }
    }
    ...
    fn cuda_fwd(
        ...
        impl Map1 for S {
            fn f<T: DeviceRepr + WithDType>(
                ...
                builder.arg(&src);
                builder.arg(&dst);
                iris_core::builder_arg!(builder, n_cols as i32);
                // SAFETY: ffi.
                unsafe { builder.launch(cfg) }.w()?;
                Ok(dst)
            }
        ...
        let dev = storage.device();
        let slice = S.map(&storage.slice, dev, layout)?;
        let dst = iris_core::cuda_backend::CudaStorage {
            slice,
            device: dev.clone(),
        };
        Ok((dst, layout.shape().clone()))
    ...
    fn metal_fwd(
        ...
        let name = match storage.dtype() {
            ...
            dtype => iris_core::bail!("softmax-last-dim is not implemented for {dtype:?}"),
        };

        let n = layout.stride().len();
        if !(layout.is_contiguous() && layout.stride()[n - 1] == 1) {
            iris_core::bail!("Non contiguous softmax-last-dim is not implemented");
        }
        ...
        let output = device
            ...
            .with_label("softmax")
            .build()?;
        iris_metal_kernels::call_last_softmax(
            device.metal_device(),
            &encoder,
            kernels,
            name,
        ...
        .map_err(iris_core::Error::wrap)?;
        let newstorage =
            iris_core::MetalStorage::new(output, device.clone(), elem_count, storage.dtype());
...
impl iris_core::CustomOp2 for RmsNorm {
    fn name(&self) -> &'static str {
        "rms-norm"
    }
    ...
    fn cpu_fwd(
        ...
        >(
            ...
            let alpha = match alpha_layout.contiguous_offsets() {
                None => iris_core::bail!("alpha has to be contiguous"),
        ...
        match (s1, s2) {
            ...
            _ => iris_core::bail!("unsupported dtype for rmsnorm {:?}", s1.dtype()),
        }
    }
    ...
    fn cuda_fwd(
        &self,
        s1: &iris_core::CudaStorage,
        l1: &Layout,
        s2: &iris_core::CudaStorage,
        l2: &Layout,
    ) -> Result<(iris_core::CudaStorage, Shape)> {
        use iris_core::cuda_backend::cudarc::driver::{
            CudaSlice, DeviceRepr, LaunchConfig, PushKernelArg,
        };
        use iris_core::cuda_backend::{kernel_name, kernels, Map2, WrapErr};
        use iris_core::{CudaDevice, WithDType};
        ...
        impl Map2 for S {
            fn f<T: DeviceRepr + WithDType>(
                ...
                builder.arg(&dst);
                builder.arg(&alpha);
                iris_core::builder_arg!(builder, n_cols as i32, block_size as i32, self.eps);
                // SAFETY: ffi.
                unsafe { builder.launch(cfg) }.w()?;
                Ok(dst)
            }
    ...
    fn metal_fwd(
        &self,
        s1: &iris_core::MetalStorage,
        l1: &Layout,
        s2: &iris_core::MetalStorage,
        l2: &Layout,
    ) -> Result<(iris_core::MetalStorage, Shape)> {
        use iris_core::backend::BackendStorage;
        ...
        let name = match (s1.dtype(), s2.dtype()) {
            ...
            (dt1, dt2) => iris_core::bail!("rmsnorm is not implemented for {dt1:?} {dt2:?}"),
        };

        if !(l1.is_contiguous() && l2.is_contiguous()) {
            iris_core::bail!("Non contiguous rmsnorm is not implemented");
        }
        ...
        let output = device
            ...
            .with_label("rmsnorm")
            .build()?;
        iris_metal_kernels::call_rms_norm(
            device.metal_device(),
            &encoder,
            kernels,
            name,
        ...
        .map_err(iris_core::Error::wrap)?;
        let newstorage = iris_core::MetalStorage::new(output, device.clone(), elem_count, s1.dtype());
...
impl iris_core::CustomOp3 for LayerNorm {
    fn name(&self) -> &'static str {
        "layer-norm"
    }
    ...
    fn cpu_fwd(
        ...
        >(
            ...
            let beta = match beta_layout.contiguous_offsets() {
                None => iris_core::bail!("beta has to be contiguous"),
    ...
    fn cuda_fwd(
        ...
        s2: &iris_core::CudaStorage,
        l2: &Layout,
        s3: &iris_core::CudaStorage,
        l3: &Layout,
    ) -> Result<(iris_core::CudaStorage, Shape)> {
        use iris_core::cuda_backend::cudarc::driver::{
            CudaSlice, DeviceRepr, LaunchConfig, PushKernelArg,
        };
        use iris_core::cuda_backend::{kernel_name, kernels, Map3, WrapErr};
        use iris_core::{CudaDevice, WithDType};
    ...
    fn metal_fwd(
        ...
        let name = match (s1.dtype(), s2.dtype(), s3.dtype()) {
            ...
            (dt1, dt2, dt3) => {
                iris_core::bail!("layernorm is not implemented for {dt1:?} {dt2:?} {dt3:?}")
            }
        };

        if !(l1.is_contiguous() && l2.is_contiguous() && l3.is_contiguous()) {
            iris_core::bail!("Non contiguous layernorm is not implemented");
        }
        ...
        let output = device
            ...
            .build()?;
        iris_metal_kernels::call_layer_norm(
            device.metal_device(),
            &encoder,
            kernels,
            name,
...
pub fn replication_pad2d(xs: &Tensor, pad: usize) -> Result<Tensor> {
    match pad {
        ...
        n => iris_core::bail!("replication-pad with a size of {n} is not supported"),
    }
}
...
impl iris_core::CustomOp3 for Sdpa {
    fn name(&self) -> &'static str {
        "metal-sdpa"
    }
    ...
    fn cpu_fwd(
        ...
        iris_core::bail!("SDPA has no cpu impl")
    }

    #[cfg(feature = "metal")]
    fn metal_fwd(
        &self,
        q: &iris_core::MetalStorage,
        q_l: &Layout,
        k: &iris_core::MetalStorage,
        k_l: &Layout,
        v: &iris_core::MetalStorage,
        v_l: &Layout,
    ) -> Result<(iris_core::MetalStorage, Shape)> {
        use iris_core::backend::BackendStorage;
        use iris_metal_kernels::SdpaDType;
        ...
        if q_l.dim(D::Minus1)? != k_l.dim(D::Minus1)? {
            iris_core::bail!("`q` and `k` last dims must match");
        }

        // k,v must have matching n kv heads
        if v_l.dim(D::Minus(3))? != k_l.dim(D::Minus(3))? {
            iris_core::bail!("`k` and `v` head dims must match");
        }

        // n_heads % n_kv_heads == 0; n_heads >= 1, n_kv_heads >= 1.
        if q_l.dim(D::Minus(3))? % k_l.dim(D::Minus(3))? != 0 {
            iris_core::bail!("query `n_heads` must be a multiple of `n_kv_heads`");
        }
        ...
        for t in [k.dtype(), v.dtype()] {
            if q.dtype() != t {
                iris_core::bail!("all q, k, v dtypes must match.");
            }
        }
        ...
        let itype = match q.dtype() {
            ...
            other => iris_core::bail!("unsupported sdpa type {other:?}"),
        };
        ...
        if supports_sdpa_vector {
            ...
            if k_seq >= TWO_PASS_K_THRESHOLD {
                let mut intermediate_shape = [
                    ...
                    &[iris_metal_kernels::SDPA_2PASS_BLOCKS],
                ...
                iris_metal_kernels::call_sdpa_vector_2pass(
                    q.device().device(),
                    &encoder,
                    q.device().kernels(),
                ...
                encoder.set_label("vector_attention");
                iris_metal_kernels::call_sdpa_vector(
                    q.device().device(),
                    &encoder,
                    q.device().kernels(),
                    q_l.start_offset() * q.dtype().size_in_bytes(),
            ...
            if self.softcapping != 1. {
                iris_core::bail!("SDPA full requires softcapping to be disabled (1.0)");
            }
            ...
            let (mask_type, mask_buffer, mask_strides) = if let Some(mask) = &self.mask {
                ...
                let mask_buffer = match &**mask_s {
                    iris_core::Storage::Metal(m) => m.buffer(),
                    _ => iris_core::bail!("Expected metal device for mask"),
                };
                ...
                if mask_type != itype {
                    iris_core::bail!("Mask type {mask_type:?} must match q type {itype:?}");
                }
            ...
            iris_metal_kernels::call_sdpa_full(
                q.device().device(),
                &encoder,
                q.device().kernels(),
                q_l.start_offset() * q.dtype().size_in_bytes(),
            ...
            iris_core::bail!("must be vector or full sdpa kernel");
        }

        let newstorage = iris_core::MetalStorage::new(output, device.clone(), elem_count, q.dtype());
