...
use iris_core::{test_utils::to_vec2_round, DType, Device, Result, Tensor};
use iris_nn::RNN;
...
fn lstm() -> Result<()> {
    ...
    .into_iter()
    .collect();
    let vb = iris_nn::VarBuilder::from_tensors(tensors, DType::F32, cpu);
    let lstm = iris_nn::lstm(2, 3, Default::default(), vb)?;
...
fn gru() -> Result<()> {
    ...
    .collect();
    let vb = iris_nn::VarBuilder::from_tensors(tensors, DType::F32, cpu);
    let gru = iris_nn::gru(2, 3, Default::default(), vb)?;
