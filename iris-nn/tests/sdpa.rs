...
mod metal_sdpa_tests {
    use iris_core::{DType, Device, Result, Shape, Tensor};
    ...
    fn sdpa_full() -> Result<()> {
        ...
        let ground_truth = {
            ...
            let att = iris_nn::ops::softmax_last_dim(&att.to_dtype(DType::F32)?)?
                .to_dtype(q.dtype())?;
            att.matmul(&v.clone())?
        };
        let sdpa_output = iris_nn::ops::sdpa(&q, &k, &v, None, false, scale as f32, 1.)?;
    ...
    fn sdpa_full_softcapping() -> Result<()> {
        ...
        let ground_truth = {
            let att = (q.clone() * scale)?.matmul(&k.clone().t()?)?;
            let att = iris_nn::ops::softmax_last_dim(
        ...
        let sdpa_output =
            iris_nn::ops::sdpa(&q, &k, &v, None, false, scale as f32, SOFTCAP as f32)?;
        assert_eq!(ground_truth.shape(), sdpa_output.shape());
        let error: f32 = ((&ground_truth - &sdpa_output)?.abs()? / &ground_truth.abs()?)?
            .sum_all()?
            .to_scalar()?;
