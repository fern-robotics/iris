...
use iris_nn::attention::flash_attn_varlen_cpu;
use iris_nn::varlen_attention::flash_attn_varlen_unfused;
...
mod tests {
    ...
    fn convert_to_precision(
        ...
        match precision {
            ...
            _ => iris_core::bail!("Unsupported precision: {:?}", precision),
        }
    }
    ...
    fn build_reference_bias(
        ...
        let slopes = if let Some(s) = alibi_slopes {
            let v = s.to_vec1::<f32>()?;
            if v.len() != num_heads {
                iris_core::bail!("alibi_slopes has len {}, expected {}", v.len(), num_heads);
            }
            Some(v)
        } else {
            None
                    ...
                    for j in 0..max_k {
                        ...
                        if window_left.is_some() || window_right.is_some() {
                            ...
                            match (window_left, window_right) {
                                ...
                                (None, None) => {}
                                (None, Some(_)) => {
                                    iris_core::bail!("window_right without window_left")
                                }
                            }
                        }
