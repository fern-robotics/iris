...
use iris_core::{test_device, test_utils::to_vec3_round, Device, IndexOp, Result, Tensor};

fn softmax(device: &Device) -> Result<()> {
    let data = &[[[3f32, 1., 4.], [1., 5., 9.]], [[2., 1., 7.], [8., 2., 8.]]];
    let tensor = Tensor::new(data, device)?;
    let t0 = iris_nn::ops::softmax(&tensor.log()?, 0)?;
    let t1 = iris_nn::ops::softmax(&tensor.log()?, 1)?;
    let t2 = iris_nn::ops::softmax(&tensor.log()?, 2)?;
    assert_eq!(
        to_vec3_round(&t0, 4)?,
        &[
            // 3/5, 1/2, 4/11
    ...
    let t2 = iris_nn::ops::softmax_last_dim(&tensor.log()?)?;
    assert_eq!(
        to_vec3_round(&t2, 4)?,
        &[
            // (3, 1, 4) / 8, (1, 5, 9) / 15
...
fn rms_norm(device: &Device) -> Result<()> {
    ...
    let t = iris_nn::ops::rms_norm(&tensor, &alpha, 1e-5)?;
    assert_eq!(
        to_vec3_round(&t, 4)?,
        &[
    ...
    let t2 = iris_nn::ops::rms_norm_slow(&tensor, &alpha, 1e-5)?;
    assert_eq!(
        to_vec3_round(&t2, 4)?,
        &[
            [[1.019, 0.6794, 4.0762], [0.1674, 1.6744, 4.521]],
...
fn rms_norml(device: &Device) -> Result<()> {
    ...
    let alpha = Tensor::ones(head_dim, iris_core::DType::F32, device)?;
    let t = iris_nn::ops::rms_norm(&tensor, &alpha, 1e-5)?;
    let t2 = iris_nn::ops::rms_norm_slow(&tensor, &alpha, 1e-5)?;
...
fn rms_norm_large_magnitude(device: &Device) -> Result<()> {
    ...
    let alpha = Tensor::ones(hidden, iris_core::DType::F32, device)?;

    let fused = iris_nn::ops::rms_norm(&tensor, &alpha, 1e-5)?;
    let slow = iris_nn::ops::rms_norm_slow(&tensor, &alpha, 1e-5)?;
...
fn layer_norm(device: &Device) -> Result<()> {
    ...
    let t = iris_nn::ops::layer_norm(&tensor, &alpha, &beta, 1e-5)?;
    assert_eq!(
        to_vec3_round(&t, 4)?,
        &[
    ...
    let t2 = iris_nn::ops::layer_norm_slow(&tensor, &alpha, &beta, 1e-5)?;
    assert_eq!(
        to_vec3_round(&t2, 4)?,
        &[
            [[0.7673, -2.6726, 3.0071], [-0.7247, 0.0, 3.4742]],
...
fn layer_norml(device: &Device) -> Result<()> {
    ...
    let tensor = Tensor::new(src, device)?.reshape((b_size, seq_len, head_dim))?;
    let alpha = Tensor::ones(head_dim, iris_core::DType::F32, device)?;
    let beta = Tensor::zeros(head_dim, iris_core::DType::F32, device)?;
    let t = iris_nn::ops::layer_norm(&tensor, &alpha, &beta, 1e-5)?;
    let t2 = iris_nn::ops::layer_norm_slow(&tensor, &alpha, &beta, 1e-5)?;
    let diff = (t - t2)?
        .abs()?
...
fn softmax_numerical_stability() -> Result<()> {
    ...
    let softmax = iris_nn::ops::softmax(&xs, 0)?;
...
fn ropei(device: &Device) -> Result<()> {
    ...
    let rope1 = iris_nn::rotary_emb::rope_i(&src, &cos, &sin)?;
    let rope2 = iris_nn::rotary_emb::rope_i_slow(&src, &cos, &sin)?;
    ...
    let rope1 = iris_nn::rotary_emb::rope_i(&src.i(0..1)?, &cos, &sin)?;
    let rope2 = iris_nn::rotary_emb::rope_i(&src.i(1..2)?, &cos2, &sin2)?;

    let both_cos = Tensor::stack(&[cos, cos2], 0)?;
    let both_sin = Tensor::stack(&[sin, sin2], 0)?;
    let both_rope = iris_nn::rotary_emb::rope_i(&src, &both_cos, &both_sin)?;
...
fn rope(device: &Device) -> Result<()> {
    ...
    let cos = Tensor::from_vec(cos, (seq_len, head_dim / 2), device)?;
    let sin = Tensor::from_vec(sin, (seq_len, head_dim / 2), device)?;
    let rope1 = iris_nn::rotary_emb::rope(&src, &cos, &sin)?;
    let rope2 = iris_nn::rotary_emb::rope_slow(&src, &cos, &sin)?;
    let sum_diff = (rope1 - rope2)?.abs()?.sum_all()?.to_vec0::<f32>()?;
    if device.is_cpu() {
        assert_eq!(sum_diff, 0.);
    } else {
    ...
    let cos2 = Tensor::from_vec(cos2, (seq_len, head_dim / 2), device)?;
    let sin2 = Tensor::from_vec(sin2, (seq_len, head_dim / 2), device)?;
    let rope1 = iris_nn::rotary_emb::rope(&src.i(0..1)?, &cos, &sin)?;
    let rope2 = iris_nn::rotary_emb::rope(&src.i(1..2)?, &cos2, &sin2)?;

    let both_cos = Tensor::stack(&[cos, cos2], 0)?;
    let both_sin = Tensor::stack(&[sin, sin2], 0)?;
    let both_rope = iris_nn::rotary_emb::rope(&src, &both_cos, &both_sin)?;
    let both_rope2 = Tensor::cat(&[rope1, rope2], 0)?;
    let sum_diff = (both_rope - both_rope2)?
        .abs()?
        .sum_all()?
...
fn rope_thd(device: &Device) -> Result<()> {
    ...
    let rope1 = {
        ...
        iris_nn::rotary_emb::rope_thd(&src, &cos, &sin)?.transpose(1, 2)?
    };
    let rope2 = iris_nn::rotary_emb::rope_slow(&src, &cos, &sin)?;
    let sum_diff = (rope1 - rope2)?.abs()?.sum_all()?.to_vec0::<f32>()?;
    if device.is_cpu() {
    ...
    let rope1 = {
        let src = src.transpose(1, 2)?.contiguous()?;
        iris_nn::rotary_emb::rope_thd(&src.i(0..1)?, &cos, &sin)?
    };
    let rope2 = {
        let src = src.transpose(1, 2)?.contiguous()?;
        iris_nn::rotary_emb::rope_thd(&src.i(1..2)?, &cos2, &sin2)?
    };
    ...
    let both_rope = {
        let src = src.transpose(1, 2)?.contiguous()?;
        iris_nn::rotary_emb::rope_thd(&src, &both_cos, &both_sin)?
    };
    let both_rope2 = Tensor::cat(&[rope1, rope2], 0)?;
    let sum_diff = (both_rope - both_rope2)?
        .abs()?
...
fn sigmoid(device: &Device) -> Result<()> {
    let data = &[[[3f32, 1., 4.], [1., 5., 9.]], [[2., 1., 7.], [8., 2., 8.]]];
    let tensor = Tensor::new(data, device)?;
    let s1 = iris_nn::ops::sigmoid(&tensor)?;
