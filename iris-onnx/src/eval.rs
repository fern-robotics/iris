...
use iris_core::Module;
use iris_core::{bail, DType, Device, IndexOp, Result, Tensor};
use iris_nn::activation::PReLU;
...
impl Attr for str {
    ...
    fn get(attr: &onnx::AttributeProto) -> Result<&Self> {
        std::str::from_utf8(&attr.s).map_err(iris_core::Error::wrap)
    }
}
...
impl Attr for GraphProto {
    ...
    fn get(attr: &onnx::AttributeProto) -> Result<&Self> {
        attr.g
            .as_ref()
            .ok_or_else(|| iris_core::Error::Msg("attribute does not contain graph".to_string()))
    }
}
...
impl AttrOwned for Vec<String> {
    ...
    fn get(attr: &onnx::AttributeProto) -> Result<Self> {
        ...
        for bytes in attr.strings.iter() {
            let s = String::from_utf8(bytes.clone()).map_err(iris_core::Error::wrap)?;
            ret.push(s);
        }
        Ok(ret)
    }
        ...
        match node.op_type.as_str() {
            ...
            "LogSoftmax" => {
                ...
                let output = match get_attr_opt::<i64>(node, "axis")? {
                    None => iris_nn::ops::softmax_last_dim(input)?,
                    Some(&axis) => {
                        let axis = input.normalize_axis(axis)?;
                        iris_nn::ops::log_softmax(input, axis)?
                    }
                };
                values.insert(node.output[0].clone(), output);
            }
            "Softmax" => {
                ...
                let output = match get_attr_opt::<i64>(node, "axis")? {
                    ...
                    Some(&axis) => {
                        let axis = input.normalize_axis(axis)?;
                        iris_nn::ops::softmax(input, axis)?
                    }
                };
                values.insert(node.output[0].clone(), output);
            }
            ...
            "Sigmoid" => {
                let input = get(&node.input[0])?;
                let output = iris_nn::ops::sigmoid(input)?;
                values.insert(node.output[0].clone(), output);
            }
            "Gelu" => {
                let input = get(&node.input[0])?;
            ...
            "LeakyRelu" => {
                ...
                let output = iris_nn::ops::leaky_relu(input, alpha.into())?;
                values.insert(node.output[0].clone(), output);
            }
            ...
            "LSTM" => {
                ...
                let vmap = iris_nn::VarMap::new();
                vmap.data().lock().unwrap().extend([
                    ("weight_ih_l0".to_string(), iris_core::Var::from_tensor(&w)?),
                    ("weight_hh_l0".to_string(), iris_core::Var::from_tensor(&r)?),
                    ("bias_ih_l0".to_string(), iris_core::Var::from_tensor(&wb)?),
                    ("bias_hh_l0".to_string(), iris_core::Var::from_tensor(&rb)?),
                ]);
                use iris_nn::rnn::RNN as _;
                let lstm = iris_nn::rnn::lstm(
                    input_size,
                    hidden_size as usize,
                    iris_nn::rnn::LSTMConfig::default(),
                    iris_nn::VarBuilder::from_varmap(&vmap, w.dtype(), w.device()),
                )?;

                let mut lstm_state = iris_nn::rnn::LSTMState::new(h, c);
            ...
            "Selu" => {
                ...
                let gamma = get_attr_opt::<f32>(node, "gamma")?
                    .copied()
                    .unwrap_or(1.050701);
                let out = iris_nn::ops::selu(input, alpha as f32, gamma as f32)?;
                values.insert(node.output[0].clone(), out);
            }
            ...
            "OneHot" => {
                ...
                if values_vec.len() != 2 {
                    return Err(iris_core::Error::Msg(
            ...
            "HardSwish" => {
                let input = get(&node.input[0])?;
                let hard_sigmoid = iris_nn::ops::hard_sigmoid(&input)?;
...
fn to_scalar_flexible<T: iris_core::WithDType>(t: &Tensor) -> Result<T> {
...
fn to_vec0_flexible<T: iris_core::WithDType>(t: &Tensor) -> Result<T> {
    if t.rank() > 0 && t.elem_count() == 1 {
