...
pub fn save_image<P: AsRef<std::path::Path>>(img: &Tensor, p: P) -> Result<()> {
    ...
    if channel != 3 {
        iris_core::bail!("save_image expects an input of shape (3, height, width)")
    }
    ...
    let image: image::ImageBuffer<image::Rgb<u8>, Vec<u8>> =
        match image::ImageBuffer::from_raw(width as u32, height as u32, pixels) {
            Some(image) => image,
            None => iris_core::bail!("error saving image {p:?}"),
        };
    image.save(p).map_err(iris_core::Error::wrap)?;
    Ok(())
}
...
pub fn hub_load_safetensors(
    ...
    let safetensors_files = safetensors_files
        .iter()
        .map(|v| repo.get(v).map_err(iris_core::Error::wrap))
...
pub fn hub_load_local_safetensors<P: AsRef<std::path::Path>>(
    ...
    let json: serde_json::Value = serde_json::from_reader(&jsfile).map_err(iris_core::Error::wrap)?;
    let weight_map = match json.get("weight_map") {
        None => iris_core::bail!("no weight map in {json_file:?}"),
        Some(serde_json::Value::Object(map)) => map,
        Some(_) => iris_core::bail!("weight map in {json_file:?} is not a map"),
