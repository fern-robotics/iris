...
pub fn pcm_decode<P: AsRef<std::path::Path>>(path: P) -> Result<(Vec<f32>, u32)> {
    ...
    let src = std::fs::File::open(path).map_err(iris_core::Error::wrap)?;
    ...
    let track = format
        ...
        .ok_or_else(|| iris_core::Error::Msg("no supported audio tracks".to_string()))?;
    ...
    let mut decoder = symphonia::default::get_codecs()
        ...
        .map_err(|_| iris_core::Error::Msg("unsupported codec".to_string()))?;
    ...
    while let Ok(packet) = format.next_packet() {
        ...
        match decoder.decode(&packet).map_err(iris_core::Error::wrap)? {
