...
Build and run the optional FA-v1 backend with both the compile-time feature and runtime switch:

```bash
cargo run --release -p iris-examples --example gemma4 --features cuda,flash-attn -- \
  --model-id google/gemma-4-E4B-it \
  --prompt 'Explain what CUDA threads are.' \
  --sample-len 64 \
  --use-flash-attn
...
Omit `--use-flash-attn` while retaining `--features cuda,flash-attn` to compare the naive and FlashAttention runtime paths from the same binary.
