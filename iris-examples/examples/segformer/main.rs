use iris_core::Device;
use iris_core::Module;
use iris_nn::VarBuilder;
use iris_transformers::models::segformer::{
...
struct SegmentationArgs {
    ...
    #[arg(
        ...
        default_value = "iris-examples/examples/segformer/assets/labels.json"
    )]
    label_path: PathBuf,
...
fn get_vb_and_config(
    ...
    let vb =
        unsafe { VarBuilder::from_mmaped_safetensors(&[model_file], iris_core::DType::F32, device)? };
...
fn segmentation_task(args: SegmentationArgs, device: &Device) -> anyhow::Result<()> {
    ...
    let image = iris_examples::imagenet::load_image224(args.image)?
        .unsqueeze(0)?
        .to_device(device)?;
...
fn classification_task(args: ClassificationArgs, device: &Device) -> anyhow::Result<()> {
    ...
    let classification = iris_nn::ops::softmax_last_dim(&classification)?;
