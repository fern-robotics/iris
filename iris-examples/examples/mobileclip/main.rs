...
use iris_transformers::models::mobileclip;
...
fn load_images<T: AsRef<std::path::Path>>(
    ...
    let mut images = vec![];
    for path in paths {
        let tensor = iris_examples::imagenet::load_image_with_std_mean(
            path,
            image_size,
            &[0.0, 0.0, 0.0],
            &[1.0, 1.0, 1.0],
...
pub fn main() -> anyhow::Result<()> {
    ...
    let vec_imgs = match args.images {
        Some(imgs) => imgs,
        None => vec![
            "iris-examples/examples/stable-diffusion/assets/stable-diffusion-xl.jpg".to_string(),
            "iris-examples/examples/yolo-v8/assets/bike.jpg".to_string(),
        ],
    };
