...
use iris_core::{DType, IndexOp, D};
use iris_nn::{Module, VarBuilder};
use iris_transformers::models::efficientnet::{EfficientNet, MBConvConfig};
...
pub fn main() -> anyhow::Result<()> {
    ...
    let image = iris_examples::imagenet::load_image224(args.image)?.to_device(&device)?;
    ...
    let model = EfficientNet::new(vb, cfg, iris_examples::imagenet::CLASS_COUNT as usize)?;
    println!("model built");
    let logits = model.forward(&image.unsqueeze(0)?)?;
    let prs = iris_nn::ops::softmax(&logits, D::Minus1)?
        .i(0)?
        .to_vec1::<f32>()?;
    ...
    for &(category_idx, pr) in prs.iter().take(5) {
        println!(
            "{:24}: {:.2}%",
            iris_examples::imagenet::CLASSES[category_idx],
            100. * pr
        );
    }
    Ok(())
