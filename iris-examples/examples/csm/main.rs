...
use iris_transformers::models::csm::{Config, Model};
...
fn main() -> Result<()> {
    ...
    let mut mimi_model = {
        use iris_transformers::models::mimi;
        let vb =
    ...
    let voices = iris_core::safetensors::load(args.voices, &device)?;
    let mut lp = iris_transformers::generation::LogitsProcessor::new(
        args.seed,
