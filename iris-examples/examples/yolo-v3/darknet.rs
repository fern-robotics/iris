use iris_core::{DType, Device, IndexOp, Result, Tensor};
use iris_nn::{batch_norm, conv2d, conv2d_no_bias, Func, Module, VarBuilder};
...
impl Block {
    fn get(&self, key: &str) -> Result<&str> {
        match self.parameters.get(key) {
            None => iris_core::bail!("cannot find {} in {}", key, self.block_type),
            Some(value) => Ok(value),
        }
    }
}
...
impl Darknet {
    fn get(&self, key: &str) -> Result<&str> {
        match self.parameters.get(key) {
            None => iris_core::bail!("cannot find {} in net parameters", key),
            Some(value) => Ok(value),
        }
    }
}
...
pub fn parse_config<T: AsRef<Path>>(path: T) -> Result<Darknet> {
    ...
    for line in BufReader::new(file).lines() {
        ...
        if line.starts_with('[') {
            if !line.ends_with(']') {
                iris_core::bail!("line does not end with ']' {line}")
            }
            ...
            if key_value.len() != 2 {
                iris_core::bail!("missing equal {line}")
            }
            ...
            if prev.is_some() {
                iris_core::bail!("multiple value for key {}", line)
            }
        }
    }
    acc.finish_block();
...
enum Bl {
    Layer(Box<dyn iris_nn::Module + Send + Sync>),
    Route(Vec<usize>),
    Shortcut(usize),
...
fn conv(vb: VarBuilder, index: usize, p: usize, b: &Block) -> Result<(usize, Bl)> {
    ...
    let conv_cfg = iris_nn::Conv2dConfig {
        stride,
        padding,
        groups: 1,
        dilation: 1,
    ...
    let leaky = match activation {
        "leaky" => true,
        "linear" => false,
        otherwise => iris_core::bail!("unsupported activation {}", otherwise),
    };
    let func = iris_nn::func(move |xs| {
...
fn yolo(p: usize, block: &Block) -> Result<(usize, Bl)> {
    ...
    if flat.len() % 2 != 0 {
        iris_core::bail!("even number of anchors");
    }
...
fn detect(
    ...
    let ys24 = xs.i((.., .., 2..4))?;
    let ys4 = xs.i((.., .., 4..))?;
    let ys02 = (iris_nn::ops::sigmoid(&ys02)?.add(&xy_offset)? * stride as f64)?;
    let ys24 = (ys24.exp()?.mul(&anchors)? * stride as f64)?;
    let ys4 = iris_nn::ops::sigmoid(&ys4)?;
    ...
    pub fn build_model(&self, vb: VarBuilder) -> Result<Func<'_>> {
        ...
        for (index, block) in self.blocks.iter().enumerate() {
            let channels_and_bl = match block.block_type.as_str() {
                ...
                otherwise => iris_core::bail!("unsupported block type {}", otherwise),
            };
