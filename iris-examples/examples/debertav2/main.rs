...
use iris_core::{Device, Tensor};
use iris_nn::ops::softmax;
use iris_nn::VarBuilder;
use iris_transformers::models::debertav2::{Config as DebertaV2Config, DebertaV2NERModel};
use iris_transformers::models::debertav2::{DebertaV2SeqClassificationModel, Id2Label};
use iris_transformers::models::debertav2::{NERItem, TextClassificationItem};
...
impl Args {
    fn build_model_and_tokenizer(
        ...
        let device = iris_examples::device(self.cpu)?;
        ...
        let mut tokenizer = Tokenizer::from_file(tokenizer_filename)
            .map_err(|e| iris_core::Error::Msg(format!("Tokenizer error: {e}")))?;
        tokenizer.with_padding(Some(PaddingParams::default()));

        let vb = if self.use_pth {
            VarBuilder::from_pth(
                &weights_filename,
                iris_transformers::models::debertav2::DTYPE,
                &device,
            )?
        } else {
            unsafe {
...
) -> impl Fn(F) -> Result<(), iris_core::Error>
where
    F: Fn(&Tensor, Tensor, Tensor) -> Result<(), iris_core::Error>,
{
    move |code: F| -> Result<(), iris_core::Error> {
