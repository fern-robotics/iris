...
use iris_transformers::models::{clip, flux, t5};

use anyhow::{Error as E, Result};
use iris_core::{IndexOp, Module, Tensor};
...
fn run(args: Args) -> Result<()> {
    ...
    let device = iris_examples::device(cpu)?;
    ...
    let img = match decode_only {
            ...
            let img = {
                ...
                let state = if quantized {
                    flux::sampling::State::new(
                        &t5_emb.to_dtype(iris_core::DType::F32)?,
                        &clip_emb.to_dtype(iris_core::DType::F32)?,
                        &img.to_dtype(iris_core::DType::F32)?,
                    )?
                } else {
        ...
        Some(file) => {
            let mut st = iris_core::safetensors::load(file, &device)?;
    ...
    println!("img\n{img}");
    let img = ((img.clamp(-1f32, 1f32)? + 1.0)? * 127.5)?.to_dtype(iris_core::DType::U8)?;
    let filename = match args.seed {
        None => "out.jpg".to_string(),
        Some(s) => format!("out-{s}.jpg"),
    };
    iris_examples::save_image(&img.i(0)?, filename)?;
    Ok(())
}
...
fn main() -> Result<()> {
    let args = Args::parse();
    #[cfg(feature = "cuda")]
    iris_core::quantized::cuda::set_force_dmmv(args.use_dmmv);
    run(args)
}
