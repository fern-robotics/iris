...
use iris_core::{DType, IndexOp, Module, Tensor, D};
use iris_transformers::models::{stable_diffusion, t5};
...
impl T5WithTokenizer {
    fn new(vb: iris_nn::VarBuilder, max_position_embeddings: usize) -> Result<Self> {
...
pub struct StableDiffusion3TripleClipWithTokenizer {
    ...
    clip_g_text_projection: iris_nn::Linear,
    t5: T5WithTokenizer,
}
...
impl StableDiffusion3TripleClipWithTokenizer {
    pub fn new_split(
        ...
        let vb_clip_g = unsafe {
            iris_nn::VarBuilder::from_mmaped_safetensors(&[clip_g_file], DType::F16, device)?
        };
        let vb_clip_l = unsafe {
            iris_nn::VarBuilder::from_mmaped_safetensors(&[clip_l_file], DType::F16, device)?
        };
        let vb_t5 = unsafe {
            iris_nn::VarBuilder::from_mmaped_safetensors(&[t5xxl_file], DType::F16, device)?
        };
        ...
        let text_projection =
            iris_nn::linear_no_bias(1280, 1280, vb_clip_g.pp("text_projection"))?;
    ...
    pub fn new(vb: iris_nn::VarBuilder) -> Result<Self> {
        let max_position_embeddings = 77usize;
        let clip_l = ClipWithTokenizer::new(
        ...
        let text_projection =
            iris_nn::linear_no_bias(1280, 1280, vb.pp("clip_g.transformer.text_projection"))?;
