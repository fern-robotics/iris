...
use iris_transformers::models::mmdit::model::{Config as MMDiTConfig, MMDiT};
...
fn main() -> Result<()> {
    ...
    let (mmdit_config, mut triple, vb) = if which.is_3_5() {
        ...
        let vb = unsafe {
            iris_nn::VarBuilder::from_mmaped_safetensors(&[model_file], DType::F16, &device)?
        };
        match which {
    ...
    iris_examples::save_image(&img.i(0)?, "out.jpg")?;
    Ok(())
}
