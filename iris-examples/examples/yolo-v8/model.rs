...
use iris_nn::{batch_norm, conv2d, conv2d_no_bias, Conv2d, Conv2dConfig, Module, VarBuilder};
...
impl Module for Dfl {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        let xs = xs
            ...
            .transpose(2, 1)?;
        let xs = iris_nn::ops::softmax(&xs, 1)?;
...
impl DetectionHead {
    ...
    fn forward(&self, xs0: &Tensor, xs1: &Tensor, xs2: &Tensor) -> Result<DetectionHeadOut> {
        ...
        let pred = Tensor::cat(&[dbox, iris_nn::ops::sigmoid(&cls)?], 1)?;
        Ok(DetectionHeadOut {
            pred,
            anchors,
            strides,
...
impl PoseHead {
    ...
    fn forward(&self, xs0: &Tensor, xs1: &Tensor, xs2: &Tensor) -> Result<Tensor> {
        ...
        let ys2 = iris_nn::ops::sigmoid(&xs.i((.., .., 2..3))?)?;
