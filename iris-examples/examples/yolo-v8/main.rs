...
use iris_transformers::object_detection::{non_maximum_suppression, Bbox, KeyPoint};
...
pub fn report_detect(
    ...
    let font = ab_glyph::FontRef::try_from_slice(&font).map_err(iris_core::Error::wrap)?;
    for (class_index, bboxes_for_class) in bboxes.iter().enumerate() {
        for b in bboxes_for_class.iter() {
            println!(
                "{}: {:?}",
                iris_examples::coco_classes::NAMES[class_index],
                b
            );
...
pub fn report_pose(
    ...
    if pred_size != 17 * 3 + 4 + 1 {
        iris_core::bail!("unexpected pred-size {pred_size}");
    }
    let mut bboxes = vec![];
    // Extract the bounding boxes for which confidence is above the threshold.
    for index in 0..npreds {
