...
use iris_transformers::generation::LogitsProcessor;
use iris_transformers::models::encodec;
use iris_transformers::models::metavoice::{adapters, gpt, tokenizers, transformer};
use iris_transformers::models::quantized_metavoice::transformer as qtransformer;

use iris_core::{DType, IndexOp, Tensor};
...
fn main() -> Result<()> {
    ...
    let mut first_stage_model = if args.quantized {
        ...
        let vb =
            iris_transformers::quantized_var_builder::VarBuilder::from_gguf(filename, &device)?;
    ...
    let spk_emb = iris_core::safetensors::load(&spk_emb_file, &iris_core::Device::Cpu)?;
    ...
    for logits in logits.iter() {
        ...
        for step in 0..seq_len {
            ...
            let prs = iris_nn::ops::softmax_last_dim(logits)?.to_vec1::<f32>()?;
    ...
    let pcm = iris_examples::audio::normalize_loudness(&pcm, 24_000, true)?;
    let pcm = pcm.to_vec1::<f32>()?;
    let mut output = std::fs::File::create(&args.out_file)?;
    iris_examples::wav::write_pcm_as_wav(&mut output, &pcm, 24_000)?;
    Ok(())
}
