...
use iris_core::{utils, DType, Device, Tensor};
use iris_nn::VarBuilder;
use iris_transformers::models::voxtral;
use iris_transformers::models::voxtral::{
...
impl VoxtralModel {
    ...
    pub fn transcribe_audio(
        ...
        let audio = if sample_rate == SAMPLE_RATE {
            ...
            iris_examples::audio::resample(audio_data, sample_rate, SAMPLE_RATE)
...
fn load_model_weights<'a>(model_files: &'a [PathBuf], device: &Device) -> Result<VarBuilder<'a>> {
    ...
    if let iris_core::Device::Cuda(_) = device {
        device.synchronize()?;
    }
