...
use iris_nn::{RmsNorm, VarBuilder};

use iris_transformers::models::with_tracing::{linear, linear_no_bias, Linear};
...
impl MambaBlock {
    pub fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        ...
        let conv_cfg = iris_nn::Conv1dConfig {
            groups: d_inner,
            padding: d_conv - 1,
            ..Default::default()
        };
        let conv1d = iris_nn::conv1d(d_inner, d_inner, d_conv, conv_cfg, vb.pp("conv1d"))?;
    ...
    fn ssm(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        let a = self.a_log.to_dtype(iris_core::DType::F32)?.exp()?.neg()?;
        let d = self.d.to_dtype(iris_core::DType::F32)?;
...
impl Module for MambaBlock {
    ...
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        let xs = xs
            ...
            .t()?;
        let xs = iris_nn::ops::silu(&xs)?;
        let ys = (self.ssm(&xs)? * iris_nn::ops::silu(res))?;
...
impl ResidualBlock {
    pub fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        let norm = iris_nn::rms_norm(cfg.d_model, 1e-5, vb.pp("norm"))?;
...
impl Model {
    pub fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        let embedding = iris_nn::embedding(cfg.vocab_size(), cfg.d_model, vb.pp("embedding"))?;
        ...
        let norm_f = iris_nn::rms_norm(cfg.d_model, 1e-5, vb.pp("norm_f"))?;
