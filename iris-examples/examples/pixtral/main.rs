...
use iris_transformers::models::pixtral::{vision_model, Config, Model};

use iris_core::{DType, Device, Module, Tensor};
...
fn main() -> Result<()> {
    ...
    let filenames = match args.weight_files {
        Some(files) => files
            ...
            .collect::<Vec<_>>(),
        None => iris_examples::hub_load_safetensors(&repo, "model.safetensors.index.json")?,
    };
    ...
    let image = if args.image.ends_with(".safetensors") {
        match iris_core::safetensors::load(&args.image, &device)?.remove("img") {
            None => anyhow::bail!("no img tensor in {}", args.image),
            Some(v) => v,
        }
    } else {
        iris_examples::imagenet::load_image_with_std_mean(
            &args.image,
            1024,
