...
pub fn main() -> Result<()> {
    ...
    match args.command {
        Command::Print { file } => {
            let model = iris_onnx::read_file(file)?;
            println!("{model:?}");
        ...
        Command::SimpleEval { file } => {
            ...
            for input in graph.input.iter() {
                ...
                let value = match type_ {
                    iris_onnx::onnx::type_proto::Value::TensorType(tt) => {
                        let dt = match DataType::try_from(tt.elem_type) {
                            Ok(dt) => match iris_onnx::dtype(dt) {
                                Some(dt) => dt,
                                None => {
                                    anyhow::bail!(
                        ...
                        let dims = shape
                                ...
                                .map(|dim| match dim.value.as_ref().expect("no dim value") {
                                    iris_onnx::onnx::tensor_shape_proto::dimension::Value::DimValue(v) => Ok(*v as usize),
                                    iris_onnx::onnx::tensor_shape_proto::dimension::Value::DimParam(_) => Ok(42),
                                })
            ...
            let outputs = iris_onnx::simple_eval(&model, inputs)?;
