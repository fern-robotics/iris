...
use iris_core::{IndexOp, D};
use iris_examples::save_image;
...
pub fn main() -> anyhow::Result<()> {
    ...
    let image = match args.which {
        Which::SqueezeNet | Which::EfficientNet => {
            iris_examples::imagenet::load_image224(&args.image)?
        }
        Which::EsrGan => iris_examples::imagenet::load_image_with_std_mean(
            &args.image,
            128,
            &[0.0f32, 0.0, 0.0],
            &[1.0f32, 1.0, 1.0],
    ...
    let model = iris_onnx::read_file(model)?;
    let graph = model.graph.as_ref().unwrap();
    let mut inputs = std::collections::HashMap::new();
    inputs.insert(graph.input[0].name.to_string(), image.unsqueeze(0)?);
    let mut outputs = iris_onnx::simple_eval(&model, inputs)?;
    let output = outputs.remove(&graph.output[0].name).unwrap();
    let prs = match args.which {
        Which::SqueezeNet => iris_nn::ops::softmax(&output, D::Minus1)?,
    ...
    match args.which {
        Which::EfficientNet | Which::SqueezeNet => {
            ...
            for &(i, p) in &top {
                println!(
                    "{:50}: {:.2}%",
                    iris_examples::imagenet::CLASSES[i],
                    p * 100.0
                );
            }
        }
        Which::EsrGan => {
            let max_pixel_val = iris_core::Tensor::try_from(255.0f32)?
                .to_device(prs.device())?
                .broadcast_as(prs.shape())?;
            let out = (prs * max_pixel_val)?.i(0)?.to_dtype(iris_core::DType::U8)?;
