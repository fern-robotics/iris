...
use iris_core::{CpuStorage, CustomOp1, DType, Device, IndexOp, Layout, Result, Shape, Tensor, D};
use iris_nn::var_builder::ShardedVarBuilder as VarBuilder;
use iris_nn::{Embedding, Linear, Module, RmsNorm};
...
pub type Config = iris_transformers::models::llama::LlamaConfig;
...
impl CustomOp1 for AllReduce {
    ...
    fn cpu_fwd(&self, _s: &CpuStorage, _l: &Layout) -> Result<(CpuStorage, Shape)> {
        iris_core::bail!("AllReduce is never used on cpu")
    }

    #[cfg(feature = "cuda")]
    fn cuda_fwd(
        &self,
        s: &iris_core::CudaStorage,
        l: &Layout,
    ) -> Result<(iris_core::CudaStorage, Shape)> {
        use iris_core::cuda_backend::WrapErr;
        ...
        let dst = match s.dtype() {
            DType::BF16 => {
                ...
                let s = match l.contiguous_offsets() {
                    ...
                    Some(_) | None => iris_core::bail!("input has to be contiguous"),
                };
                let mut dst = unsafe { dev.alloc::<bf16>(elem_count) }.w()?;
                self.comm
                    .all_reduce(s, &mut dst, &ReduceOp::Sum)
                    .map_err(iris_core::Error::debug)?;
                iris_core::CudaStorage::wrap_cuda_slice(dst, dev)
            }
            DType::F16 => {
            ...
            dtype => iris_core::bail!("unsupported dtype {dtype:?}"),
        };
...
fn shard(dim: usize, rank: usize, world_size: usize) -> iris_nn::var_builder::Shard {
    iris_nn::var_builder::Shard {
        dim,
        rank,
        world_size,
    }
...
impl CausalSelfAttention {
    fn apply_rotary_emb(&self, x: &Tensor, index_pos: usize) -> Result<Tensor> {
        ...
        iris_nn::rotary_emb::rope(x, &cos, &sin)
    }
    ...
    fn forward(&self, x: &Tensor, index_pos: usize, block_idx: usize) -> Result<Tensor> {
        ...
        let y = iris_flash_attn::flash_attn(&q, &k, &v, softmax_scale, seq_len > 1)?
    ...
    fn repeat_kv(&self, x: Tensor) -> Result<Tensor> {
        ...
        iris_transformers::utils::repeat_kv(x, n_rep)
    }
