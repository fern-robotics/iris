...
use iris_transformers::models::llama::LlamaEosToks;
...
fn main() -> Result<()> {
    ...
    let filenames = iris_examples::hub_load_safetensors(&api, "model.safetensors.index.json")?;
    ...
    let vb = unsafe {
        iris_nn::var_builder::ShardedSafeTensors::var_builder(&filenames, dtype, &device)?
    };
