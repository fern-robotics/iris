...
use iris_transformers::models::qwen2::{Config as ConfigBase, ModelForCausalLM as ModelBase};
use iris_transformers::models::qwen2_moe::{Config as ConfigMoe, Model as ModelMoe};
use iris_transformers::models::qwen3::{Config as Config3, ModelForCausalLM as Model3};
use iris_transformers::models::qwen3_moe::{Config as ConfigMoe3, ModelForCausalLM as ModelMoe3};
...
impl Model {
    fn forward(&mut self, xs: &Tensor, s: usize) -> iris_core::Result<Tensor> {
        match self {
    ...
    let filenames = match args.weight_path {
        Some(path) => {
            if std::path::Path::new(&path)
                ...
                .exists()
            {
                iris_examples::hub_load_local_safetensors(path, "model.safetensors.index.json")?
            } else {
        ...
        None => match args.model {
            ...
            | WhichModel::W3_8b
            | WhichModel::W3MoeA3b => {
                iris_examples::hub_load_safetensors(&repo, "model.safetensors.index.json")?
            }
        },
    };
