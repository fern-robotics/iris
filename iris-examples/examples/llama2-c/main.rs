...
use iris_transformers::models::llama2_c as model;
use iris_transformers::models::llama2_c_weights as weights;
use iris_transformers::models::quantized_llama2_c as qmodel;
mod training;
use clap::{Parser, Subcommand};

use anyhow::{Error as E, Result};
use byteorder::{LittleEndian, ReadBytesExt};
use iris_core::{IndexOp, Tensor};
...
fn run_inference(args: &InferenceCmd, common_args: &Args) -> Result<()> {
    ...
    let device = iris_examples::device(common_args.cpu)?;
    #[cfg(feature = "cuda")]
    if let iris_core::Device::Cuda(d) = &device {
        unsafe {
    ...
    let (model, config, mut cache) = if is_gguf {
        ...
        let fake_vb = iris_nn::VarBuilder::from_tensors(
            [
        ...
        let tensors = iris_core::safetensors::load(config_path, &device)?;
        let vb = iris_nn::VarBuilder::from_tensors(tensors, iris_core::DType::F32, &device);
