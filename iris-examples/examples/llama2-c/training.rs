...
use iris_core::{DType, Device, Result};
use iris_datasets::nlp::tinystories::{Dataset, DatasetRandomIter};
use iris_nn::Optimizer;
...
fn valid_loss(
    ...
    let batch_iter = iris_datasets::Batcher::new_r2(iter).batch_size(args.batch_size);
    let mut sum_ce = 0f64;
    let mut cnt = 0usize;
    for inp_tgt in batch_iter.take(50) {
        let (inp, tgt) = inp_tgt?;
        let logits = model.forward(&inp, 0, cache)?;
        let loss = iris_nn::loss::cross_entropy(&logits.flatten_to(1)?, &tgt.flatten_to(1)?)?;
...
pub fn run(args: &crate::TrainingCmd, common_args: &crate::Args) -> Result<()> {
    let device = iris_examples::device(common_args.cpu)?;
    ...
    let varmap = iris_nn::VarMap::new();
    let vb = iris_nn::VarBuilder::from_varmap(&varmap, DType::F32, &device);
    ...
    let params = iris_nn::ParamsAdamW {
        lr: args.learning_rate,
        ..Default::default()
    };
    let mut opt = iris_nn::AdamW::new(varmap.all_vars(), params)?;
