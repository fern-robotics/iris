...
use iris_transformers::models::xlm_roberta::{
...
fn main() -> Result<()> {
    ...
    let vb = if weights_filename.ends_with("model.safetensors") {
        unsafe {
            VarBuilder::from_mmaped_safetensors(&[weights_filename], iris_core::DType::F16, &device)
                .unwrap()
        }
    } else {
        println!("Loading weights from pytorch_model.bin");
        VarBuilder::from_pth(&weights_filename, iris_core::DType::F16, &device).unwrap()
    };
    tokenizer
    ...
    match args.task {
        ...
        Task::Reranker => {
            ...
            let output = iris_nn::ops::sigmoid(&output)?.t().unwrap();
            let ranks = output
...
pub fn tokenize_batch(
    ...
    let token_ids = tokens
        ...
        .collect::<iris_core::Result<Vec<_>>>()?;
