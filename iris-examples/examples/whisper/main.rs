...
use iris_core::{Device, IndexOp, Tensor};
use iris_nn::{
...
use iris_transformers::models::whisper::{self as m, audio, Config};
...
impl Model {
    ...
    pub fn encoder_forward(&mut self, x: &Tensor, flush: bool) -> iris_core::Result<Tensor> {
        match self {
    ...
    pub fn decoder_final_linear(&self, x: &Tensor) -> iris_core::Result<Tensor> {
        match self {
...
impl Decoder {
    ...
    fn decode(&mut self, mel: &Tensor, t: f64) -> Result<DecodingResult> {
        ...
        for i in 0..sample_len {
            ...
            let prob = softmax(&logits, iris_core::D::Minus1)?
...
pub fn token_id(tokenizer: &Tokenizer, token: &str) -> iris_core::Result<u32> {
    match tokenizer.token_to_id(token) {
        None => iris_core::bail!("no token-id for {token}"),
        Some(id) => Ok(id),
    }
}
...
struct Args {
    ...
    /// repo: https://huggingface.co/datasets/Narsil/iris_demo/
    #[arg(long)]
    input: Option<String>,
...
fn main() -> Result<()> {
    ...
    let (config_filename, tokenizer_filename, weights_filename, input) = {
        ...
        let sample = if let Some(input) = args.input {
            ...
            println!("No audio file submitted: Downloading https://huggingface.co/datasets/Narsil/iris_demo/blob/main/samples_jfk.wav");
    ...
    let (pcm_data, sample_rate) = iris_examples::audio::pcm_decode(input)?;
    ...
    let mut model = if args.quantized {
        let vb = iris_transformers::quantized_var_builder::VarBuilder::from_gguf(
            &weights_filename,
            &device,
        )?;
