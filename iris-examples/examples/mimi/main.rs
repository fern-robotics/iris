...
use iris_transformers::models::mimi::{Config, Model};
...
fn main() -> Result<()> {
    ...
    let codes = match args.action {
        Action::CodeToAudio => {
            let codes = iris_core::safetensors::load(args.in_file, &device)?;
        ...
        Action::AudioToCode | Action::AudioToAudio => {
            ...
            match args.streaming {
                Some(chunk_size) => {
                    ...
                    Tensor::cat(&code_chunks, iris_core::D::Minus1)?
                }
                None => {
                    let pcm_len = pcm.len();
    ...
    match args.action {
        ...
        Action::AudioToAudio | Action::CodeToAudio => {
            let pcm = match args.streaming {
                Some(chunk_size) => {
                    let seq_len = codes.dim(iris_core::D::Minus1)?;
                    ...
                    for chunk_start in (0..seq_len).step_by(chunk_size) {
                        ...
                        let codes = codes.narrow(iris_core::D::Minus1, chunk_start, chunk_len)?;
                        let pcm = model.decode_step(&codes.into())?;
                        if let Some(pcm) = pcm.as_option() {
                            pcm_chunks.push(pcm.clone())
                        }
                    }
                    Tensor::cat(&pcm_chunks, iris_core::D::Minus1)?
                }
