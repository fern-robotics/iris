...
use iris_transformers::models::llama::{Cache, Llama, LlamaConfig};
use iris_transformers::models::snac::{Config as SnacConfig, Model as SnacModel};
...
struct Model {
    model: Llama,
    tokenizer: Tokenizer,
    logits_processor: iris_transformers::generation::LogitsProcessor,
    cache: Cache,
    device: Device,
    verbose_prompt: bool,
    snac: SnacModel,
...
impl Model {
    ...
    fn run(&mut self, prompt: &str) -> Result<()> {
        ...
        iris_examples::wav::write_pcm_as_wav(&mut output, &pcm, 24000)?;
        Ok(())
    }
}
