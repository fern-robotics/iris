...
use iris_core::{quantized::QTensor, DType, Result, Tensor, D};
use iris_nn::{linear_no_bias, moe, Activation, Linear, VarBuilder};
use std::sync::Arc;
...
impl FusedMoe {
    ...
    pub fn forward(&self, xs: &Tensor, is_prefill: bool) -> Result<Tensor> {
        ...
        let routing_weights =
            iris_nn::ops::softmax_last_dim(&router_logits.to_dtype(DType::F32)?)?;
        ...
        let gate = gate_up
            .narrow(iris_core::D::Minus1, 0, self.w_size_n)?
            .contiguous()?;
        let up = gate_up
            .narrow(iris_core::D::Minus1, self.w_size_n, self.w_size_n)?
            .contiguous()?;
