...
use iris_core::{DType, Device, IndexOp, Module, Result, Tensor, D};
use iris_nn::{
...
pub fn find_audio_token_positions(
    ...
    let input_ids = if input_ids.dtype() == iris_core::DType::U32 {
        input_ids.to_dtype(iris_core::DType::I64)?
    } else {
        input_ids.clone()
    };
...
impl Module for VoxtralAttention {
    fn forward(&self, x: &Tensor) -> Result<Tensor> {
        ...
        let attn_weights = iris_nn::ops::softmax_last_dim(&scores)?;
...
impl VoxtralEncoderLayer {
    fn new(cfg: &VoxtralEncoderConfig, vb: VarBuilder) -> Result<Self> {
        ...
        let activation = match cfg.activation_function.as_str() {
            "gelu" => iris_nn::Activation::Gelu,
            "relu" => iris_nn::Activation::Relu,
            _ => iris_core::bail!(
...
impl VoxtralEncoder {
    ...
    pub fn forward_with_training(&self, input_features: &Tensor, training: bool) -> Result<Tensor> {
        ...
        let x = if false {
            ...
            let x_f32 = x.to_dtype(iris_core::DType::F32)?;
            let positions_f32 = positions.to_dtype(iris_core::DType::F32)?;
...
impl VoxtralForConditionalGeneration {
    ...
    pub fn get_audio_embeds(&self, input_features: &Tensor) -> Result<Tensor> {
        ...
        if total_elements % self.audio_config.intermediate_size != 0 {
            return Err(iris_core::Error::DimOutOfRange {
                shape: iris_core::Shape::from_dims(&[batch_size, seq_len, hidden_size]),
                dim: 0,
                op: "reshape",
            });
        }
    ...
    pub fn generate(
        ...
        if let Some(p) = config.top_p {
            if !(0.0..=1.0).contains(&p) {
                iris_core::bail!("top_p must be between 0 and 1, got {}", p);
            }
        }
        ...
        for idx in 0..config.max_new_tokens {
            ...
            let next_token = if config.temperature > 0.0 {
                ...
                let prs = iris_nn::ops::softmax_last_dim(&prs)?;
                ...
                let argmax_result = match logits.argmax(D::Minus1) {
                    Ok(result) => result,
                    Err(e) => {
                        return Err(iris_core::Error::Msg(format!("Argmax failed: {e}")));
                    }
                };
                ...
                if argmax_result.dims().is_empty() {
                    ...
                    match argmax_result.to_scalar::<u32>() {
                        Ok(token) => token,
                        Err(e) => {
                            return Err(iris_core::Error::Msg(format!("to_scalar failed: {e}")));
                        }
                    }
