...
use iris_core::quantized::QMatMul;
use iris_core::{bail, DType, Device, IndexOp, Result, Tensor};
use iris_nn::{Conv1d, Conv1dConfig, Embedding, Module};
...
fn get_qtensor<R: std::io::Seek + std::io::Read>(
    ...
    device: &Device,
    names: &[String],
) -> Result<iris_core::quantized::QTensor> {
    for name in names {
...
impl Module for Mlp {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        self.w2.forward(&(iris_nn::ops::silu(&w1)? * w3)?)
    }
}
...
impl AttentionLayer {
    fn apply_rotary_emb(&self, x: &Tensor, index_pos: usize) -> Result<Tensor> {
        ...
        iris_nn::rotary_emb::rope(&x.contiguous()?, &cos, &sin)
    }
