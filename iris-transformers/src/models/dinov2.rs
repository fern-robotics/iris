...
//!   -- --color-map \
//!   --image iris-examples/examples/yolo-v8/assets/bike.jpg
//! ```
//!
...
//!
//! <div align=center>
//!   <img src="https://github.com/huggingface/candle/raw/main/iris-examples/examples/yolo-v8/assets/bike.jpg" alt="" width=640>
//! </div>
//!
//! ```bash
//! cargo run \
...
use iris_nn::{layer_norm, LayerNorm, Linear, Module, VarBuilder};
...
fn linear(vb: VarBuilder, in_dim: usize, out_dim: usize, bias: bool) -> Result<Linear> {
    if bias {
        iris_nn::linear(in_dim, out_dim, vb)
    } else {
        iris_nn::linear_no_bias(in_dim, out_dim, vb)
    }
}
...
impl Module for Attention {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        let attn = iris_nn::ops::softmax(&q.matmul(&k.t()?)?, D::Minus1)?;
...
impl PatchEmbed {
    fn new(
        ...
        let config = iris_nn::Conv2dConfig {
            stride: patch_size,
            ..Default::default()
        };
        let proj = iris_nn::conv2d(in_chans, embed_dim, patch_size, config, vb.pp("proj"))?;
...
impl Module for PatchEmbed {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        if (h % patch_h) != 0 {
            iris_core::bail!("image height {h} is not a multiple of patch height {patch_h}")
        }
        if (w % patch_w) != 0 {
            iris_core::bail!("image width {w} is not a multiple of patch width {patch_w}")
        }
