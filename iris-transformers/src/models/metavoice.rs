...
use iris_core::{DType, Device, Error as E, IndexOp, Module, Result, Tensor, D};
use iris_nn::{embedding, linear_b, rms_norm, Embedding, Linear, RmsNorm, VarBuilder};
    ...
    impl Model {
        pub fn new(cfg: Config, vb: VarBuilder) -> Result<Self> {
            ...
            for layer_idx in 0..cfg.model_num_layers {
                let c = iris_nn::LSTMConfig {
                    layer_idx,
                    ..Default::default()
                };
                let lstm = iris_nn::lstm(
                    cfg.mel_n_channels,
        ...
        pub fn embed_utterance(
            ...
            let max_wave_length = match wav_slices.last() {
                Some(v) => v.1,
                None => iris_core::bail!("empty wav slices"),
            };
    ...
    impl BPE {
        pub fn from_json(json: &serde_json::Value, end_of_text: usize) -> Result<Self> {
            let json = match json.as_object() {
                None => iris_core::bail!("json value is not an object"),
                Some(json) => json,
            };
            let re = match json.get("pat_str") {
                None => iris_core::bail!("json object has no pat_str field"),
                Some(pat_str) => match pat_str.as_str() {
                    None => iris_core::bail!("pat_str field is not a string"),
                    Some(pat_str) => fancy_regex::Regex::new(pat_str).map_err(E::wrap)?,
                },
            };
            let offset = match json.get("offset") {
                None => iris_core::bail!("json object has no offset field"),
                Some(offset) => match offset.as_u64() {
                    None => iris_core::bail!("offset field is not a positive int"),
            ...
            let mergeable_ranks = match json.get("mergeable_ranks") {
                None => iris_core::bail!("json object has no mergeable_ranks field"),
                Some(mr) => match mr.as_object() {
                    None => iris_core::bail!("mergeable_ranks is not an object"),
                    Some(mr) => mr,
                },
            };
            for (key, value) in mergeable_ranks.iter() {
                let value = match value.as_u64() {
                    None => iris_core::bail!("mergeable_ranks '{key}' is not a u64"),
...
pub mod gpt {
    ...
    enum Norm {
        RMSNorm(iris_nn::RmsNorm),
        LayerNorm(iris_nn::LayerNorm),
    }
    ...
    impl Norm {
        fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
            match cfg.norm_type {
                NormType::RMSNorm => {
                    let rms_norm = iris_nn::rms_norm(cfg.n_embd, cfg.rmsnorm_eps, vb)?;
                    Ok(Self::RMSNorm(rms_norm))
                }
                NormType::LayerNorm => {
                    let ln_cfg = iris_nn::LayerNormConfig {
                        affine: cfg.bias,
                        ..Default::default()
                    };
                    let layer_norm = iris_nn::layer_norm(cfg.n_embd, ln_cfg, vb)?;
    ...
    impl SelfAttention {
        fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
            ...
            if cfg.attn_kernel_type != AttnKernelType::TorchAttn {
                iris_core::bail!("only TorchAttn is supported")
            }
            if cfg.kv_cache_enabled {
                iris_core::bail!("kv_cache_enabled=true is not supported")
            }
            ...
            let slf = match cfg.nonlinearity_type {
                ...
                NonLinearityType::Swiglu => {
                    ...
                    let swiglu_multiple_of = match cfg.swiglu_multiple_of {
                        None => iris_core::bail!("swiglu-multiple-of has to be set"),
                        Some(smo) => smo,
                    };
    ...
    #[allow(clippy::upper_case_acronyms)]
    pub struct Model {
        wtes: Vec<iris_nn::Embedding>,
        wpe: iris_nn::Embedding,
        h: Vec<Block>,
        ln_f: Norm,
        lm_heads: Vec<Linear>,
        cfg: Config,
    ...
    impl Model {
        pub fn new(cfg: Config, vb: VarBuilder) -> Result<Self> {
            ...
            for (idx, vocab_size) in cfg.vocab_sizes.iter().enumerate() {
                let wte = iris_nn::embedding(*vocab_size, cfg.n_embd, vb_w.pp(idx))?;
                wtes.push(wte)
            }
            let wpe = iris_nn::embedding(cfg.block_size, cfg.n_embd, vb_t.pp("wpe"))?;
...
pub mod transformer {
    ...
    impl Module for FeedForward {
        fn forward(&self, xs: &Tensor) -> Result<Tensor> {
            let _enter = self.span.enter();
            let swiglu = (iris_nn::ops::silu(&xs.apply(&self.w1)?)? * xs.apply(&self.w3))?;
