...
pub struct AttnProjections {
    head_dim: usize,
    qkv: nn::Linear,
    ln_k: Option<iris_nn::RmsNorm>,
    ln_q: Option<iris_nn::RmsNorm>,
    proj: nn::Linear,
}
...
impl AttnProjections {
    pub fn new(dim: usize, num_heads: usize, vb: nn::VarBuilder) -> Result<Self> {
        ...
        let (ln_k, ln_q) = if vb.contains_tensor("ln_k.weight") {
            let ln_k = iris_nn::rms_norm(head_dim, 1e-6, vb.pp("ln_k"))?;
            let ln_q = iris_nn::rms_norm(head_dim, 1e-6, vb.pp("ln_q"))?;
            (Some(ln_k), Some(ln_q))
        } else {
            (None, None)
        };
