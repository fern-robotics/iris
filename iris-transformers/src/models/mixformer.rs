...
impl RotaryEmbedding {
    ...
    fn apply_rotary_emb_qkv(
        ...
        if three != 3 {
            iris_core::bail!("unexpected shape for qkv {:?}", qkv.shape())
        }
        ...
        let q_rot = iris_nn::rotary_emb::rope_thd(&q_rot, &c, &s)?;
        let k_rot = iris_nn::rotary_emb::rope_thd(&k_rot, &c, &s)?;
...
#[derive(Debug, Clone)]
struct CausalLMHead {
    ln: iris_nn::LayerNorm,
    linear: Linear,
}

impl CausalLMHead {
    fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        let ln = iris_nn::layer_norm(cfg.n_embd, cfg.layer_norm_epsilon, vb.pp("ln"))?;
...
impl MHA {
    ...
    fn forward(&mut self, xs: &Tensor, mask: Option<&Tensor>) -> Result<Tensor> {
        ...
        let attn_weights = {
            ...
            iris_nn::ops::softmax_last_dim(&attn_weights)?
        };
