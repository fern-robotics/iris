use iris_core::{DType, IndexOp, Result, Tensor, D};
use iris_nn::{LayerNorm, Linear, RmsNorm, VarBuilder};
...
fn scaled_dot_product_attention(q: &Tensor, k: &Tensor, v: &Tensor) -> Result<Tensor> {
    ...
    let attn_scores = iris_nn::ops::softmax_last_dim(&attn_weights)?.matmul(&v)?;
...
pub(crate) fn timestep_embedding(t: &Tensor, dim: usize, dtype: DType) -> Result<Tensor> {
    ...
    if dim % 2 == 1 {
        iris_core::bail!("{dim} is odd")
    }
    let dev = t.device();
    let half = dim / 2;
    let t = (t * TIME_FACTOR)?;
    let arange = Tensor::arange(0, half as u32, dev)?.to_dtype(iris_core::DType::F32)?;
    let freqs = (arange * (-MAX_PERIOD.ln() / half as f64))?.exp()?;
    let args = t
        .unsqueeze(1)?
        .to_dtype(iris_core::DType::F32)?
...
impl iris_core::Module for EmbedNd {
...
impl MlpEmbedder {
    fn new(in_sz: usize, h_sz: usize, vb: VarBuilder) -> Result<Self> {
        let in_layer = iris_nn::linear(in_sz, h_sz, vb.pp("in_layer"))?;
        let out_layer = iris_nn::linear(h_sz, h_sz, vb.pp("out_layer"))?;
        Ok(Self {
            in_layer,
            out_layer,
        })
...
impl iris_core::Module for MlpEmbedder {
...
impl Modulation1 {
    fn new(dim: usize, vb: VarBuilder) -> Result<Self> {
        let lin = iris_nn::linear(dim, 3 * dim, vb.pp("lin"))?;
        Ok(Self { lin })
    }
    ...
    fn forward(&self, vec_: &Tensor) -> Result<ModulationOut> {
        let ys = vec_
            ...
            .chunk(3, D::Minus1)?;
        if ys.len() != 3 {
            iris_core::bail!("unexpected len from chunk {ys:?}")
        }
        Ok(ModulationOut {
            shift: ys[0].clone(),
            scale: ys[1].clone(),
...
impl Modulation2 {
    fn new(dim: usize, vb: VarBuilder) -> Result<Self> {
        let lin = iris_nn::linear(dim, 6 * dim, vb.pp("lin"))?;
        Ok(Self { lin })
    }
...
impl SelfAttention {
    fn new(dim: usize, num_heads: usize, qkv_bias: bool, vb: VarBuilder) -> Result<Self> {
        ...
        let qkv = iris_nn::linear_b(dim, dim * 3, qkv_bias, vb.pp("qkv"))?;
        let norm = QkNorm::new(head_dim, vb.pp("norm"))?;
        let proj = iris_nn::linear(dim, dim, vb.pp("proj"))?;
        Ok(Self {
            qkv,
            norm,
            proj,
...
impl Mlp {
    fn new(in_sz: usize, mlp_sz: usize, vb: VarBuilder) -> Result<Self> {
        let lin1 = iris_nn::linear(in_sz, mlp_sz, vb.pp("0"))?;
        let lin2 = iris_nn::linear(mlp_sz, in_sz, vb.pp("2"))?;
        Ok(Self { lin1, lin2 })
    }
}

impl iris_core::Module for Mlp {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
...
impl SingleStreamBlock {
    fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        ...
        let linear1 = iris_nn::linear(h_sz, h_sz * 3 + mlp_sz, vb.pp("linear1"))?;
        let linear2 = iris_nn::linear(h_sz + mlp_sz, h_sz, vb.pp("linear2"))?;
        let norm = QkNorm::new(head_dim, vb.pp("norm"))?;
...
impl LastLayer {
    fn new(h_sz: usize, p_sz: usize, out_c: usize, vb: VarBuilder) -> Result<Self> {
        ...
        let linear = iris_nn::linear(h_sz, p_sz * p_sz * out_c, vb.pp("linear"))?;
        let ada_ln_modulation = iris_nn::linear(h_sz, 2 * h_sz, vb.pp("adaLN_modulation.1"))?;
        Ok(Self {
            norm_final,
            linear,
            ada_ln_modulation,
...
impl Flux {
    pub fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        let img_in = iris_nn::linear(cfg.in_channels, cfg.hidden_size, vb.pp("img_in"))?;
        let txt_in = iris_nn::linear(cfg.context_in_dim, cfg.hidden_size, vb.pp("txt_in"))?;
...
impl super::WithForward for Flux {
    ...
    fn forward(
        ...
        if txt.rank() != 3 {
            iris_core::bail!("unexpected shape for txt {:?}", txt.shape())
        }
        if img.rank() != 3 {
            iris_core::bail!("unexpected shape for img {:?}", img.shape())
        }
        let dtype = img.dtype();
        let pe = {
