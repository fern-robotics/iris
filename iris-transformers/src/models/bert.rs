...
//! - See bert in [candle-examples](https://github.com/huggingface/candle/tree/main/iris-examples/) for runnable code
//!
use super::with_tracing::{layer_norm, linear, LayerNorm, Linear};
use iris_core::{DType, Device, Result, Tensor};
...
impl HiddenActLayer {
    ...
    fn forward(&self, xs: &Tensor) -> iris_core::Result<Tensor> {
...
impl BertSelfAttention {
    ...
    fn forward(&self, hidden_states: &Tensor, attention_mask: &Tensor) -> Result<Tensor> {
        ...
        let attention_probs = {
            ...
            iris_nn::ops::softmax(&attention_scores, iris_core::D::Minus1)?
        };
        let attention_probs = self.dropout.forward(&attention_probs)?;

        let context_layer = attention_probs.matmul(&value_layer)?;
        let context_layer = context_layer.transpose(1, 2)?.contiguous()?;
        let context_layer = context_layer.flatten_from(iris_core::D::Minus2)?;
        Ok(context_layer)
    }
}
...
fn get_extended_attention_mask(attention_mask: &Tensor, dtype: DType) -> Result<Tensor> {
    let attention_mask = match attention_mask.rank() {
        ...
        _ => iris_core::bail!("Wrong shape for input_ids or attention_mask"),
    };
