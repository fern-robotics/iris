...
use iris_nn::{embedding, Embedding, Module, VarBuilder};
...
impl SelfAttention {
    ...
    pub fn forward(&self, xs: &Tensor, state: &mut State) -> Result<Tensor> {
        ...
        let (receptance, key, value, gate) = {
            ...
            let gate = iris_nn::ops::silu(&self.gate.forward(&gate)?)?;
...
impl FeedForward {
    ...
    pub fn forward(&self, xs: &Tensor, state: &mut State) -> Result<Tensor> {
        ...
        let receptance = iris_nn::ops::sigmoid(&receptance.apply(&self.receptance)?)?;
...
impl Tokenizer {
    pub fn new<P: AsRef<std::path::Path>>(p: P) -> Result<Self> {
        ...
        let token2idx: HashMap<String, u32> =
            serde_json::from_reader(file).map_err(iris_core::Error::wrap)?;
    ...
    pub fn decode(&self, tokens: &[u32]) -> Result<String> {
        ...
        String::from_utf8(bytes).map_err(iris_core::Error::wrap)
    }
    ...
    pub fn encode_bytes(&self, bytes: &[u8]) -> Result<Vec<u32>> {
        ...
        while i < bytes.len() {
            ...
            let token = match self.token2idx.get(&s) {
                None => iris_core::bail!("unexpected token '{}' {s:?}", String::from_utf8_lossy(&s)),
                Some(token) => *token,
            };
            tokens.push(token)
        }
