...
use iris_nn::{Conv1d, Conv1dConfig, ConvTranspose1d, ConvTranspose1dConfig, VarBuilder};
...
impl iris_core::Module for Snake1d {
...
impl iris_core::Module for ResidualUnit {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        let ys = xs
            .apply(&self.snake1)?
            .apply(&self.conv1)?
...
impl iris_core::Module for EncoderBlock {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        xs.apply(&self.res1)?
            .apply(&self.res2)?
            .apply(&self.res3)?
...
impl iris_core::Module for Encoder {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
...
impl iris_nn::Module for DecoderBlock {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
...
impl iris_core::Module for Decoder {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        let mut xs = xs.apply(&self.conv1)?;
        for block in self.blocks.iter() {
            xs = xs.apply(block)?
...
impl VectorQuantizer {
    pub fn new(in_dim: usize, cb_size: usize, cb_dim: usize, vb: VarBuilder) -> Result<Self> {
        ...
        let codebook = iris_nn::embedding(cb_size, cb_dim, vb.pp("codebook"))?;
        Ok(Self {
            in_proj,
            out_proj,
            codebook,
...
impl ResidualVectorQuantizer {
    ...
    pub fn from_codes(&self, codes: &Tensor) -> Result<Tensor> {
        ...
        match sum {
            Some(s) => Ok(s),
            None => iris_core::bail!("empty codebooks"),
        }
    }
}
