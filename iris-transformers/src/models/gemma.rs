...
use iris_nn::{linear_b as linear, Activation, Linear, VarBuilder};
...
impl Config {
    fn hidden_act(&self) -> Result<Activation> {
        match (self.hidden_act, self.hidden_activation) {
            ...
            (Some(_), Some(_)) => iris_core::bail!("both hidden_act and hidden_activation are set"),
            (None, None) => iris_core::bail!("none of hidden_act and hidden_activation are set"),
        }
    }
}
...
impl Model {
    ...
    pub fn embed_tokens(&self) -> &iris_nn::Embedding {
        &self.embed_tokens
    }
