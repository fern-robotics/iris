...
use iris_nn::{linear_b as linear, Linear, VarBuilder};
use std::sync::Arc;
...
pub struct Config {
    ...
    pub hidden_activation: iris_nn::Activation,
...
impl RotaryEmbedding {
    pub(crate) fn new(dtype: DType, cfg: &Config, dev: &Device) -> Result<Self> {
        if cfg.partial_rotary_factor != 0.5 {
            iris_core::bail!("partial-rotary-factor {} <> 0.5", cfg.partial_rotary_factor)
        }
...
impl Rglru {
    ...
    pub(crate) fn forward(&mut self, xs: &Tensor, pos: usize) -> Result<Tensor> {
        ...
        let input_gate = iris_nn::ops::sigmoid(&input_gate)?;
        let res = baddbmm(
        ...
        let recurrent_gate = iris_nn::ops::sigmoid(&recurrent_gate)?;
    ...
    pub fn forward(&mut self, xs: &Tensor, pos: usize) -> Result<Tensor> {
        ...
        let x_branch = if pos == 0 {
            ...
            let conv_state = match self.conv1d_state.as_ref() {
                None => iris_core::bail!("empty cache despite pos > 0"),
