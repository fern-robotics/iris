...
use iris_core::{DType, Module, Result, Tensor, D};
use iris_nn::{Conv1d, Conv2d, Conv2dConfig, VarBuilder};
...
impl SSCPConvBlock {
    fn new(
        ...
        let conv = iris_nn::conv2d_no_bias(
            in_channels,
            out_channels,
...
struct SubSampleConvProjection {
    ...
    input_proj_linear: iris_nn::Linear,
}
...
impl SubSampleConvProjection {
    fn new(cfg: &Gemma4AudioConfig, vb: VarBuilder) -> Result<Self> {
        ...
        let input_proj_linear = iris_nn::linear_no_bias(
...
impl RelativePositionEmbedding {
    fn new(cfg: &Gemma4AudioConfig, vb: VarBuilder) -> Result<Self> {
        ...
        let pos_proj =
            iris_nn::linear_no_bias(channels, num_heads * head_dim, vb.pp("relative_k_proj"))?;
...
impl ConformerAttention {
    fn new(cfg: &Gemma4AudioConfig, vb: VarBuilder) -> Result<Self> {
        ...
        let q_proj =
            iris_nn::linear_no_bias(hidden_size, num_heads * head_dim, attn_vb.pp("q_proj"))?;
        let k_proj =
            iris_nn::linear_no_bias(hidden_size, num_heads * head_dim, attn_vb.pp("k_proj"))?;
        let v_proj =
            iris_nn::linear_no_bias(hidden_size, num_heads * head_dim, attn_vb.pp("v_proj"))?;
        let post = iris_nn::linear_no_bias(hidden_size, hidden_size, attn_vb.pp("post"))?;
    ...
    fn forward(&self, x: &Tensor, mask: &Tensor) -> Result<Tensor> {
        ...
        let probabilities = iris_nn::ops::softmax_last_dim(&masked_logits.to_dtype(DType::F32)?)?;
...
struct ConformerFeedForward {
    ...
    ffw_layer_1: iris_nn::Linear,
    ffw_layer_2: iris_nn::Linear,
...
impl ConformerFeedForward {
    fn new(cfg: &Gemma4AudioConfig, vb: VarBuilder) -> Result<Self> {
        Ok(Self {
            ...
            ffw_layer_1: iris_nn::linear_no_bias(
                cfg.hidden_size,
                cfg.hidden_size * 4,
                vb.pp("ffw_layer_1"),
            )?,
            ffw_layer_2: iris_nn::linear_no_bias(
                cfg.hidden_size * 4,
                cfg.hidden_size,
                vb.pp("ffw_layer_2"),
            )?,
    ...
    fn forward(&self, x: &Tensor) -> Result<Tensor> {
        ...
        let x = iris_nn::ops::silu(&self.ffw_layer_1.forward(&x)?)?;
        let x = self
            .ffw_layer_2
            .forward(&x)?
            .clamp(-self.gradient_clipping, self.gradient_clipping)?;
...
struct ConformerLightConv1d {
    ...
    conv_norm: RmsNorm,
    linear_start: iris_nn::Linear,
    linear_end: iris_nn::Linear,
...
impl ConformerLightConv1d {
    fn new(cfg: &Gemma4AudioConfig, vb: VarBuilder) -> Result<Self> {
        Ok(Self {
            ...
            linear_start: iris_nn::linear_no_bias(
                cfg.hidden_size,
                cfg.hidden_size * 2,
                vb.pp("linear_start"),
            )?,
            depthwise_conv1d: iris_nn::conv1d_no_bias(
                cfg.hidden_size,
                cfg.hidden_size,
                cfg.conf_conv_kernel_size,
                iris_nn::Conv1dConfig {
                    stride: 1,
                    padding: 0,
                    dilation: 1,
            ...
            linear_end: iris_nn::linear_no_bias(
                cfg.hidden_size,
                cfg.hidden_size,
                vb.pp("linear_end"),
            )?,
    ...
    fn forward(&self, audio_encodings: &Tensor) -> Result<Tensor> {
        ...
        let x = (x1 * iris_nn::ops::sigmoid(&x2)?)?;
...
pub struct AudioModel {
    ...
    output_proj: Option<iris_nn::Linear>,
}
