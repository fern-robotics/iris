...
impl CoreAttention {
    ...
    fn forward(
        ...
        let attention_probs = iris_nn::ops::softmax_last_dim(&attention_scores)?;
...
impl SelfAttention {
    ...
    fn forward(
        ...
        if !self.multi_query_attention {
            iris_core::bail!("only multi_query_attention=true is supported")
        }
...
impl Module for MLP {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        xs.apply(&self.dense_h_to_4h)?
            .apply(&iris_nn::Activation::Swiglu)?
...
#[derive(Debug, Clone)]
struct Block {
    input_layernorm: iris_nn::LayerNorm,
    self_attention: SelfAttention,
    post_attention_layernorm: iris_nn::LayerNorm,
    mlp: MLP,
...
struct Transformer {
    layers: Vec<Block>,
    final_layernorm: Option<iris_nn::LayerNorm>,
    rotary_emb: RotaryEmbedding,
}
...
#[derive(Debug, Clone)]
struct Embedding {
    word_embeddings: iris_nn::Embedding,
    fp32_residual_connection: bool,
}

impl Embedding {
    fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        let word_embeddings = iris_nn::embedding(
...
impl Module for Embedding {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        if self.fp32_residual_connection {
            xs.to_dtype(iris_core::DType::F32)
        } else {
            xs.contiguous()
        }
    }
