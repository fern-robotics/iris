...
use iris_core::{IndexOp, Module, Result, Tensor, D};
use iris_nn::{layer_norm, linear, LayerNorm, Linear, VarBuilder};
...
fn default_text_hidden_act() -> iris_nn::Activation {
    iris_nn::Activation::GeluPytorchTanh
}
...
pub struct TextConfig {
    ...
    pub hidden_act: iris_nn::Activation,
...
fn default_vision_hidden_act() -> iris_nn::Activation {
    iris_nn::Activation::GeluPytorchTanh
}
...
trait TransformerConfig {
    ...
    fn hidden_act(&self) -> iris_nn::Activation;
}
...
impl VisionConfig {
    pub fn paligemma_3b_224() -> Self {
        Self {
            ...
            // Default values.
            num_channels: 3,
            hidden_act: iris_nn::Activation::GeluPytorchTanh,
            layer_norm_eps: 1e-6,
        }
    }
...
impl MultiheadAttention {
    ...
    fn forward(&self, q: &Tensor, k: &Tensor, v: &Tensor) -> Result<Tensor> {
        ...
        let attn = iris_nn::ops::softmax_last_dim(&attn)?;
...
impl Attention {
    ...
    fn forward(&self, xs: &Tensor, attention_mask: Option<&Tensor>) -> Result<Tensor> {
        ...
        // The original implementation upcasts to f32 but iris_nn::ops::softmax should handle this properly.
...
struct Mlp {
    fc1: Linear,
    fc2: Linear,
    activation_fn: iris_nn::Activation,
}
...
impl Mlp {
    fn new<C: TransformerConfig>(cfg: &C, vb: VarBuilder) -> Result<Self> {
        ...
        let fc1 = iris_nn::linear(hidden_size, intermediate_size, vb.pp("fc1"))?;
        let fc2 = iris_nn::linear(intermediate_size, hidden_size, vb.pp("fc2"))?;
        Ok(Self {
            fc1,
            fc2,
...
impl Module for Mlp {
    fn forward(&self, xs: &iris_core::Tensor) -> Result<iris_core::Tensor> {
        xs.apply(&self.fc1)?
...
struct VisionEmbeddings {
    patch_embedding: iris_nn::Conv2d,
...
impl VisionEmbeddings {
    fn new(cfg: &VisionConfig, vb: VarBuilder) -> Result<Self> {
        let conv2d_cfg = iris_nn::Conv2dConfig {
            stride: cfg.patch_size,
            ..Default::default()
        };
        let patch_embedding = iris_nn::conv2d(
            cfg.num_channels,
            cfg.hidden_size,
            cfg.patch_size,
            conv2d_cfg,
        ...
        let embedder = iris_nn::embedding(
...
impl TextEmbeddings {
    fn new(cfg: &TextConfig, vb: VarBuilder) -> Result<Self> {
        let token_embedding =
            iris_nn::embedding(cfg.vocab_size, cfg.hidden_size, vb.pp("token_embedding"))?;
