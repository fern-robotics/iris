...
use iris_core::{bail, Context, DType, Device, Module, Result, Tensor, D};
use iris_nn::{
...
pub struct DebertaV2Embeddings {
    ...
    embed_proj: Option<iris_nn::Linear>,
}
...
impl DebertaV2Embeddings {
    pub fn load(vb: VarBuilder, config: &Config) -> Result<Self> {
        ...
        let embed_proj: Option<iris_nn::Linear> = if embedding_size != config.hidden_size {
...
impl XSoftmax {
    pub fn apply(input: &Tensor, mask: &Tensor, dim: D, device: &Device) -> Result<Tensor> {
        ...
        output = iris_nn::ops::softmax(&output, dim)?;
...
pub struct DebertaV2DisentangledSelfAttention {
    ...
    query_proj: iris_nn::Linear,
    key_proj: iris_nn::Linear,
    value_proj: iris_nn::Linear,
    dropout: StableDropout,
    device: Device,
    ...
    pos_ebd_size: isize,
    share_att_key: bool,
    pos_key_proj: Option<iris_nn::Linear>,
    pos_query_proj: Option<iris_nn::Linear>,
}
...
impl DebertaV2DisentangledSelfAttention {
    pub fn load(vb: VarBuilder, config: &Config) -> Result<Self> {
        ...
        {
            return Err(iris_core::Error::Msg(format!(
        ...
        let query_proj = iris_nn::linear(config.hidden_size, all_head_size, vb.pp("query_proj"))?;
        let key_proj = iris_nn::linear(config.hidden_size, all_head_size, vb.pp("key_proj"))?;
        let value_proj = iris_nn::linear(config.hidden_size, all_head_size, vb.pp("value_proj"))?;
        ...
        let mut pos_key_proj: Option<iris_nn::Linear> = None;
        let mut pos_query_proj: Option<iris_nn::Linear> = None;
        ...
        if relative_attention {
            ...
            if !share_att_key {
                if config.pos_att_type.iter().any(|s| s == "c2p") {
                    pos_key_proj = Some(iris_nn::linear(
                        config.hidden_size,
                        all_head_size,
                        vb.pp("pos_key_proj"),
                    )?);
                ...
                if config.pos_att_type.iter().any(|s| s == "p2c") {
                    pos_query_proj = Some(iris_nn::linear(
                        config.hidden_size,
                        all_head_size,
...
impl DebertaV2SelfOutput {
    pub fn load(vb: VarBuilder, config: &Config) -> Result<Self> {
        let dense = iris_nn::linear(config.hidden_size, config.hidden_size, vb.pp("dense"))?;
        let layer_norm = iris_nn::layer_norm(
            config.hidden_size,
            config.layer_norm_eps,
            vb.pp("LayerNorm"),
        )?;
...
pub struct DebertaV2NERModel {
    ...
    dropout: iris_nn::Dropout,
    classifier: iris_nn::Linear,
}
...
impl DebertaV2NERModel {
    pub fn load(vb: VarBuilder, config: &Config, id2label: Option<Id2Label>) -> Result<Self> {
        ...
        let dropout = iris_nn::Dropout::new(config.hidden_dropout_prob as f32);
        let classifier: iris_nn::Linear = iris_nn::linear_no_bias(
            config.hidden_size,
            id2label_len,
...
impl DebertaV2SeqClassificationModel {
    pub fn load(vb: VarBuilder, config: &Config, id2label: Option<Id2Label>) -> Result<Self> {
        ...
        let classifier = iris_nn::linear(output_dim, id2label_len, vb.root().pp("classifier"))?;
...
pub(crate) fn make_log_bucket_position(
    ...
    let condition = lt_mid
        ...
        .mul(&gt_neg_mid.to_dtype(iris_core::DType::F32)?)?
        .to_dtype(DType::U8)?;
