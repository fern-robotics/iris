...
use iris_core::{Result, D};
use iris_nn::{batch_norm, Conv2d, Func, VarBuilder};
...
fn conv2d(
    ...
    iris_nn::conv2d_no_bias(c_in, c_out, ksize, conv2d_cfg, vb)
}
...
fn resnet(
    ...
    let fc = match nclasses {
        None => None,
        Some(nclasses) => {
            let linear = iris_nn::linear(512, nclasses, vb.pp("fc"))?;
            Some(linear)
        }
    };
    Ok(Func::new(move |xs| {
...
fn bottleneck_resnet(
    ...
    let fc = match nclasses {
        None => None,
        Some(nclasses) => {
            let linear = iris_nn::linear(4 * 512, nclasses, vb.pp("fc"))?;
            Some(linear)
        }
    };
    Ok(Func::new(move |xs| {
