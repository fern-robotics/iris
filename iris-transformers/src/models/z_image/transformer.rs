...
use iris_nn::{linear, linear_no_bias, VarBuilder};
...
fn flash_attn(_: &Tensor, _: &Tensor, _: &Tensor, _: f32, _: bool) -> Result<Tensor> {
    iris_core::bail!("flash-attn feature not enabled, compile with '--features flash-attn'")
}
...
pub struct TimestepEmbedder {
    linear1: iris_nn::Linear,
    linear2: iris_nn::Linear,
...
impl ZImageAttention {
    ...
    fn attention_metal(
        ...
        iris_nn::ops::sdpa(q, k, v, sdpa_mask.as_ref(), false, scale as f32, 1.0)
    }
    ...
    fn attention_basic(
        ...
        let attn_probs = iris_nn::ops::softmax_last_dim(&attn_weights)?;
        attn_probs.matmul(v)
    }
...
pub struct ZImageTransformerBlock {
    ...
    ffn_norm1: RmsNorm,
    ffn_norm2: RmsNorm,
    adaln_modulation: Option<iris_nn::Linear>,
}
...
pub struct FinalLayer {
    ...
    linear: iris_nn::Linear,
    adaln_silu: iris_nn::Linear,
}

impl FinalLayer {
    pub fn new(hidden_size: usize, out_channels: usize, vb: VarBuilder) -> Result<Self> {
        let norm_final = LayerNormNoParams::new(1e-6);
        let linear = iris_nn::linear(hidden_size, out_channels, vb.pp("linear"))?;
        let adaln_dim = hidden_size.min(ADALN_EMBED_DIM);
        let adaln_silu =
            iris_nn::linear(adaln_dim, hidden_size, vb.pp("adaLN_modulation").pp("1"))?;
...
pub struct ZImageTransformer2DModel {
    ...
    cap_embedder_linear: iris_nn::Linear,
    x_embedder: iris_nn::Linear,
