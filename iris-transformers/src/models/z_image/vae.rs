...
use iris_core::{Module, Result, Tensor, D};
use iris_nn::{conv2d, group_norm, Conv2d, Conv2dConfig, GroupNorm, VarBuilder};
...
impl Attention {
    fn new(channels: usize, num_groups: usize, vb: VarBuilder) -> Result<Self> {
        ...
        let to_q = iris_nn::linear(channels, channels, vb.pp("to_q"))?;
        let to_k = iris_nn::linear(channels, channels, vb.pp("to_k"))?;
        let to_v = iris_nn::linear(channels, channels, vb.pp("to_v"))?;
        let to_out = iris_nn::linear(channels, channels, vb.pp("to_out").pp("0"))?;
        Ok(Self {
            group_norm,
            to_q,
            to_k,
