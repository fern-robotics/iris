...
use iris_core::{DType, Device, Module, Result, Tensor};
use iris_nn::{Activation, VarBuilder};
use std::sync::Arc;
...
#[derive(Debug, Clone)]
struct Mlp {
    gate_proj: iris_nn::Linear,
    up_proj: iris_nn::Linear,
    down_proj: iris_nn::Linear,
    act_fn: Activation,
}
...
impl Mlp {
    fn new(cfg: &TextEncoderConfig, vb: VarBuilder) -> Result<Self> {
        Ok(Self {
            gate_proj: iris_nn::linear_no_bias(
                cfg.hidden_size,
                cfg.intermediate_size,
                vb.pp("gate_proj"),
            )?,
            up_proj: iris_nn::linear_no_bias(
                cfg.hidden_size,
                cfg.intermediate_size,
                vb.pp("up_proj"),
            )?,
            down_proj: iris_nn::linear_no_bias(
                cfg.intermediate_size,
                cfg.hidden_size,
                vb.pp("down_proj"),
            )?,
...
impl ZImageTextEncoder {
    pub fn new(cfg: &TextEncoderConfig, vb: VarBuilder) -> Result<Self> {
        ...
        let embed_tokens =
            iris_nn::embedding(cfg.vocab_size, cfg.hidden_size, vb_model.pp("embed_tokens"))?;
    ...
    pub fn forward(&self, input_ids: &Tensor) -> Result<Tensor> {
        ...
        iris_core::bail!("Layer index out of bounds")
    }
