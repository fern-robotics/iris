...
pub struct Config {
    pub projector_hidden_act: iris_nn::Activation,
...
impl Model {
    pub fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        ...
        let vision_tower = vision_model::Model::new(
            &cfg.vision_config,
            vb.pp("vision_tower").to_dtype(iris_core::DType::F32),
        )?;
        let multi_modal_projector = MultiModalProjector::new(
            cfg,
            vb.pp("multi_modal_projector").to_dtype(iris_core::DType::F32),
        )?;
        Ok(Self {
