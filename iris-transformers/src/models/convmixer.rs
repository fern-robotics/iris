...
use iris_nn::{batch_norm, Conv2dConfig, Module, VarBuilder};
...
fn conv2d_same(
    ...
    let conv2d = iris_nn::conv2d(i, o, k, c, vb)?;
    let s = c.stride;
    let module = iris_nn::func(move |xs| {
        let ih = xs.dim(2)?;
        let iw = xs.dim(3)?;
...
fn block(dim: usize, kernel_size: usize, vb: VarBuilder) -> Result<impl Module> {
    ...
    let conv2 = iris_nn::conv2d(dim, dim, 1, Default::default(), vb.pp(1))?;
    let bn2 = batch_norm(dim, 1e-5, vb.pp(3))?;
    Ok(iris_nn::func(move |xs| {
...
fn convmixer(
    ...
    patch_size: usize,
    vb: VarBuilder,
) -> Result<iris_nn::Func<'static>> {
    let conv2d_cfg = Conv2dConfig {
        stride: patch_size,
        ..Default::default()
    };
    let conv1 = iris_nn::conv2d(3, dim, patch_size, conv2d_cfg, vb.pp(0))?;
    let bn1 = batch_norm(dim, 1e-5, vb.pp(2))?;
    let blocks: Vec<_> = (0..depth)
        .map(|index| block(dim, kernel_size, vb.pp(3 + index)))
        .collect::<Result<Vec<_>>>()?;
    let fc = iris_nn::linear(dim, nclasses, vb.pp(25))?;
    Ok(iris_nn::func(move |xs| {
...
pub fn c1536_20(nclasses: usize, vb: VarBuilder) -> Result<iris_nn::Func<'static>> {
    convmixer(nclasses, 1536, 20, 9, 7, vb)
}

pub fn c1024_20(nclasses: usize, vb: VarBuilder) -> Result<iris_nn::Func<'static>> {
