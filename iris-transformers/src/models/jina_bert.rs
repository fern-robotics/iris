...
impl Config {
    pub fn v2_base() -> Self {
        ...
        Self {
            ...
            hidden_act: iris_nn::Activation::Gelu,
...
impl BertSelfAttention {
    ...
    fn forward(&self, xs: &Tensor, bias: &Tensor) -> Result<Tensor> {
        ...
        let attention_probs = {
            ...
            iris_nn::ops::softmax_last_dim(&attention_scores)?
        };
...
impl BertGLUMLP {
    fn new(vb: VarBuilder, cfg: &Config) -> Result<Self> {
        ...
        let act = iris_nn::Activation::Gelu; // geglu
...
impl BertEncoder {
    fn new(vb: VarBuilder, cfg: &Config) -> Result<Self> {
        if cfg.position_embedding_type != PositionEmbeddingType::Alibi {
            iris_core::bail!("only alibi is supported as a position-embedding-type")
        }
