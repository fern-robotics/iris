...
use iris_core::{DType, Device, IndexOp, Module, Result, StreamTensor, StreamingModule, Tensor, D};
use iris_nn::{linear_no_bias, Linear, VarBuilder};
use std::sync::Arc;

fn linear(in_d: usize, out_d: usize, bias: bool, vb: VarBuilder) -> Result<Linear> {
    if bias {
        iris_nn::linear(in_d, out_d, vb)
    } else {
...
pub struct Config {
    ...
    pub gating: Option<iris_nn::Activation>,
...
impl RotaryEmbedding {
    ...
    pub fn apply_rotary_emb(&self, qk: &Tensor, seqlen_offset: usize) -> Result<Tensor> {
        ...
        iris_nn::rotary_emb::rope_i(&qk.to_dtype(DType::F32)?, &c, &s)?.to_dtype(qk_dtype)
    }
}
...
pub struct StreamingMultiheadAttention {
    ...
    kv_cache: iris_nn::kv_cache::RotatingKvCache,
    pos: usize,
    use_flash_attn: bool,
    span: tracing::Span,
}
...
impl StreamingMultiheadAttention {
    pub fn new(rope: &Option<Arc<RotaryEmbedding>>, cfg: &Config, vb: VarBuilder) -> Result<Self> {
        ...
        Ok(Self {
            ...
            context: cfg.context,
            neg_inf,
            kv_cache: iris_nn::kv_cache::RotatingKvCache::new(2, cfg.context),
            pos: 0,
    ...
    pub fn forward(&mut self, xs: &Tensor, mask: Option<&Tensor>) -> Result<Tensor> {
        let _enter = self.span.enter();
        if self.kv_repeat != 1 {
            iris_core::bail!("only kv-repeat = 1 is supported")
        }
        let (b, t, hd) = xs.dims3()?;
        ...
        let xs = if q.dtype() == DType::BF16 && self.use_flash_attn {
            ...
            let ws = iris_nn::ops::softmax_last_dim(&pre_ws)?; // b,h,t,k
            ws.matmul(&v)? // b,h,t,d
        };
        let xs = xs
    ...
    pub fn set_kv_cache(&mut self, kv_cache: iris_nn::kv_cache::RotatingKvCache) {
...
impl Module for RmsNorm {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        iris_nn::ops::rms_norm(xs, &self.alpha, self.eps)
    }
}
    ...
    pub fn new(d_model: usize, cfg: &Config, vb: VarBuilder) -> Result<Self> {
        let norm = match cfg.norm {
            super::NormType::LayerNorm => {
                let norm = iris_nn::layer_norm(d_model, 1e-5, vb)?;
                Self::LayerNorm(norm)
            }
...
pub struct StreamingTransformerLayer {
    ...
    cross_attn: Option<(iris_nn::LayerNorm, StreamingMultiheadCrossAttention)>,
    norm_first: bool,
    span: tracing::Span,
}
...
impl StreamingTransformerLayer {
    pub fn new(rope: &Option<Arc<RotaryEmbedding>>, cfg: &Config, vb: VarBuilder) -> Result<Self> {
        if cfg.use_conv_block {
            iris_core::bail!("conv-block is not supported")
        }
        let d_model = cfg.d_model;
        let mlp = Mlp::new(cfg, vb.clone())?;
        let (norm1, norm2) = match cfg.norm {
            super::NormType::LayerNorm => {
                let norm1 = iris_nn::layer_norm(d_model, 1e-5, vb.pp("input_layernorm"))?;
                let norm2 =
                    iris_nn::layer_norm(d_model, 1e-5, vb.pp("post_attention_layernorm"))?;
        ...
        let cross_attn = if cfg.cross_attention {
            let norm_cross = iris_nn::layer_norm(cfg.d_model, 1e-5, vb.pp("norm_cross"))?;
    ...
    pub fn forward(
        ...
        let _enter = self.span.enter();
        if !self.norm_first {
            iris_core::bail!("only norm_first = true is supported")
        }
...
impl StreamingTransformer {
    ...
    pub fn copy_state(&mut self, from: &Self) -> Result<()> {
        if self.layers.len() != from.layers.len() {
            iris_core::bail!("cannot copy kv-caches as the transformers have different depths")
        }
        self.layers
            .iter_mut()
