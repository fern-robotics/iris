...
use iris_core::{Module, Result, StreamTensor, StreamingModule, Tensor, D};
use iris_nn::{Conv1d, VarBuilder};
...
impl NormConv1d {
    ...
    pub fn new(
        ...
        norm: Option<Norm>,
        bias: bool,
        cfg: iris_nn::Conv1dConfig,
        vb: VarBuilder,
    ) -> Result<Self> {
        let conv = match norm {
            None | Some(Norm::TimeGroupNorm) => {
                if bias {
                    iris_nn::conv1d(in_c, out_c, k_size, cfg, vb.pp("conv"))?
                } else {
                    iris_nn::conv1d_no_bias(in_c, out_c, k_size, cfg, vb.pp("conv"))?
                }
            }
            Some(Norm::WeightNorm) => {
                conv1d_weight_norm(in_c, out_c, k_size, bias, cfg, vb.pp("conv"))?
            }
            Some(Norm::SpectralNorm) => iris_core::bail!("SpectralNorm is not supported yet."),
        };
        let norm = match norm {
            None | Some(Norm::WeightNorm) | Some(Norm::SpectralNorm) => None,
            Some(Norm::TimeGroupNorm) => {
                if causal {
                    iris_core::bail!("GroupNorm doesn't support causal evaluation.")
                }
                let norm = iris_nn::group_norm(1, out_c, 1e-5, vb.pp("norm"))?;
                Some(norm)
            }
        };
        Ok(Self {
...
fn unpad1d(xs: &Tensor, unpad_l: usize, unpad_r: usize) -> Result<Tensor> {
    ...
    if len < unpad_l + unpad_r {
        iris_core::bail!("unpad1d: tensor len {len} is too low, {unpad_l} + {unpad_r}")
    }
...
impl StreamableConv1d {
    ...
    pub fn new(
        ...
        if k_size < stride {
            iris_core::bail!("kernel-size {k_size} is smaller than stride {stride}")
        }
        Ok(Self {
            conv,
            causal,
...
impl ConvDownsample1d {
    pub fn new(
        ...
        if !learnt {
            iris_core::bail!("only learnt=true is supported")
        }
...
mod tests {
    ...
    fn run_conv1d(
        ...
        let dev = &iris_core::Device::Cpu;
        let vm = iris_nn::VarMap::new();
        let vb = VarBuilder::from_varmap(&vm, iris_core::DType::F32, dev);
        ...
        if diff > 1e-5 {
            ...
            println!("{ys}");
            println!("{ys_steps}");
            iris_core::bail!("larger diff than expected {diff}")
        }
        Ok(())
    }
