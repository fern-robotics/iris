...
use iris_core::{IndexOp, Layout, Result, Shape, Tensor, D};
use iris_nn::{linear, Linear, VarBuilder};
...
impl EuclideanCodebook {
    ...
    pub fn decode(&self, indexes: &Tensor) -> Result<Tensor> {
        ...
        // let ys = iris_nn::Embedding::new(self.embedding.clone(), self.dim).forward(xs)?;
...
impl ResidualVectorQuantization {
    ...
    pub fn decode(&self, xs: &Tensor) -> Result<Tensor> {
        if self.layers.is_empty() {
            iris_core::bail!("empty layers in ResidualVectorQuantization")
        }
...
pub struct ResidualVectorQuantizer {
    ...
    input_proj: Option<iris_nn::Conv1d>,
    output_proj: Option<iris_nn::Conv1d>,
}
