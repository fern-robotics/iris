...
use iris_core::{DType, Device, Module, Result, StreamTensor, StreamingModule, Tensor};
...
impl Config {
    ...
    pub fn v0_1(num_codebooks: Option<usize>) -> Self {
        let seanet_cfg = seanet::Config {
            ...
            n_filters: 64,
            n_residual_layers: 1,
            activation: iris_nn::Activation::Elu(1.),
            compress: 2,
            dilation_base: 2,
...
pub fn load(model_file: &str, num_codebooks: Option<usize>, dev: &Device) -> Result<Encodec> {
    let vb =
        unsafe { iris_nn::VarBuilder::from_mmaped_safetensors(&[model_file], DType::F32, dev)? };
