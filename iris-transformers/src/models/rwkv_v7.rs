...
use iris_nn::{embedding, Embedding, VarBuilder};
...
fn layer_norm(xs: &Tensor, weight: &Tensor, bias: &Tensor, eps: f64) -> Result<Tensor> {
    ...
    let dim = xs_f32.dim(iris_core::D::Minus1)?;
    let mean = (xs_f32.sum_keepdim(iris_core::D::Minus1)? / dim as f64)?;
    let centered = xs_f32.broadcast_sub(&mean)?;
    let var = (centered.sqr()?.sum_keepdim(iris_core::D::Minus1)? / dim as f64)?;
    ...
    fn forward(
        ...
        let (v, v_first) = if self.layer_id == 0 {
            ...
            if let (Some(v0), Some(v1), Some(v2)) = (&self.v0, &self.v1, &self.v2) {
                let gate = iris_nn::ops::sigmoid(&(v0 + mm!(mm!(xv, v1), v2))?)?;
        ...
        let a = iris_nn::ops::sigmoid(&(&self.a0 + mm!(mm!(xa, &self.a1), &self.a2))?)?;

        // 6. Gate: g = sigmoid(xg @ g1) @ g2
        let g = mm!(iris_nn::ops::sigmoid(&mm!(xg, &self.g1))?, &self.g2);
...
impl Model {
    ...
    pub fn forward_seq(&self, token_ids: &[u32], state: &mut State) -> Result<Tensor> {
        if token_ids.is_empty() {
            iris_core::bail!("token_ids cannot be empty");
        }
        ...
        last_logits.ok_or_else(|| iris_core::Error::Msg("No tokens processed".to_string()))
    }
}
