...
#[derive(Debug)]
pub struct WPrior {
    projection: iris_nn::Conv2d,
    cond_mapper_lin1: iris_nn::Linear,
    cond_mapper_lin2: iris_nn::Linear,
    blocks: Vec<Block>,
    out_ln: super::common::WLayerNorm,
    out_conv: iris_nn::Conv2d,
    c_r: usize,
}
...
impl WPrior {
    ...
    pub fn new(
        ...
        let projection = iris_nn::conv2d(c_in, c, 1, Default::default(), vb.pp("projection"))?;
        let cond_mapper_lin1 = iris_nn::linear(c_cond, c, vb.pp("cond_mapper.0"))?;
        let cond_mapper_lin2 = iris_nn::linear(c, c, vb.pp("cond_mapper.2"))?;
        let out_ln = super::common::WLayerNorm::new(c)?;
        let out_conv = iris_nn::conv2d(c, c_in * 2, 1, Default::default(), vb.pp("out.1"))?;
    ...
    pub fn forward(&self, xs: &Tensor, r: &Tensor, c: &Tensor) -> Result<Tensor> {
        ...
        let c_embed = c
            ...
            .apply(&|xs: &_| iris_nn::ops::leaky_relu(xs, 0.2))?
