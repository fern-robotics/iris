...
pub struct TimestepBlock {
    mapper: iris_nn::Linear,
}

impl TimestepBlock {
    pub fn new(c: usize, c_timestep: usize, vb: VarBuilder) -> Result<Self> {
        let mapper = iris_nn::linear(c_timestep, c * 2, vb.pp("mapper"))?;
        Ok(Self { mapper })
    }
...
impl ResBlock {
    pub fn new(c: usize, c_skip: usize, ksize: usize, vb: VarBuilder) -> Result<Self> {
        ...
        let depthwise = iris_nn::conv2d(c + c_skip, c, ksize, cfg, vb.pp("depthwise"))?;
        let norm = WLayerNorm::new(c)?;
        let channelwise_lin1 = iris_nn::linear(c, c * 4, vb.pp("channelwise.0"))?;
...
impl AttnBlock {
    pub fn new(
        ...
        let kv_mapper_lin = iris_nn::linear(c_cond, c, vb.pp("kv_mapper.1"))?;
        Ok(Self {
            self_attn,
            norm,
            attention,
    ...
    pub fn forward(&self, xs: &Tensor, kv: &Tensor) -> Result<Tensor> {
        let kv = iris_nn::ops::silu(kv)?.apply(&self.kv_mapper_lin)?;
