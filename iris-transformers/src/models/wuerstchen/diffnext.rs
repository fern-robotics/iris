...
impl ResBlockStageB {
    pub fn new(c: usize, c_skip: usize, ksize: usize, vb: VarBuilder) -> Result<Self> {
        ...
        let depthwise = iris_nn::conv2d(c, c, ksize, cfg, vb.pp("depthwise"))?;
        let norm = WLayerNorm::new(c)?;
        let channelwise_lin1 = iris_nn::linear(c + c_skip, c * 4, vb.pp("channelwise.0"))?;
        let channelwise_grn = GlobalResponseNorm::new(4 * c, vb.pp("channelwise.2"))?;
        let channelwise_lin2 = iris_nn::linear(c * 4, c, vb.pp("channelwise.4"))?;
        Ok(Self {
            depthwise,
            norm,
            channelwise_lin1,
...
struct UpBlock {
    sub_blocks: Vec<SubBlock>,
    layer_norm: Option<WLayerNorm>,
    conv: Option<iris_nn::ConvTranspose2d>,
}

#[derive(Debug)]
pub struct WDiffNeXt {
    clip_mapper: iris_nn::Linear,
    effnet_mappers: Vec<Option<iris_nn::Conv2d>>,
    seq_norm: LayerNormNoWeights,
    embedding_conv: iris_nn::Conv2d,
    embedding_ln: WLayerNorm,
    down_blocks: Vec<DownBlock>,
    up_blocks: Vec<UpBlock>,
    clf_ln: WLayerNorm,
    clf_conv: iris_nn::Conv2d,
    c_r: usize,
    patch_size: usize,
}
...
impl WDiffNeXt {
    ...
    pub fn new(
        ...
        let clip_mapper = iris_nn::linear(clip_embd, c_cond, vb.pp("clip_mapper"))?;
        ...
        let embedding_conv = iris_nn::conv2d(
        ...
        for (i, &c_hidden) in C_HIDDEN.iter().enumerate() {
            ...
            let (layer_norm, conv, start_layer_i) = if i > 0 {
                ...
                let conv = iris_nn::conv2d(C_HIDDEN[i - 1], c_hidden, 2, cfg, vb.pp("0.1"))?;
        ...
        for (i, &c_hidden) in C_HIDDEN.iter().enumerate().rev() {
            ...
            let (layer_norm, conv) = if i > 0 {
                ...
                let conv = iris_nn::conv_transpose2d(
                    c_hidden,
                    C_HIDDEN[i - 1],
                    2,
                    cfg,
    ...
    pub fn forward(
        ...
        let mut xs = xs
            .apply(&|xs: &_| iris_nn::ops::pixel_unshuffle(xs, self.patch_size))?
        ...
        let ab = xs
            .apply(&self.clf_ln)?
            .apply(&self.clf_conv)?
            .apply(&|xs: &_| iris_nn::ops::pixel_shuffle(xs, self.patch_size))?
            .chunk(2, 1)?;
        let b = ((iris_nn::ops::sigmoid(&ab[1])? * (1. - EPS * 2.))? + EPS)?;
        (x_in - &ab[0])? / b
    }
}
