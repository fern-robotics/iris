...
use iris_core::{Module, Result, Tensor};
use iris_nn::VarBuilder;

#[derive(Debug)]
pub struct MixingResidualBlock {
    norm1: LayerNormNoWeights,
    depthwise_conv: iris_nn::Conv2d,
    norm2: LayerNormNoWeights,
    channelwise_lin1: iris_nn::Linear,
    channelwise_lin2: iris_nn::Linear,
    gammas: Vec<f32>,
}
...
impl MixingResidualBlock {
    pub fn new(inp: usize, embed_dim: usize, vb: VarBuilder) -> Result<Self> {
        ...
        let cfg = iris_nn::Conv2dConfig {
            groups: inp,
            ..Default::default()
        };
        let depthwise_conv = iris_nn::conv2d(inp, inp, 3, cfg, vb.pp("depthwise.1"))?;
        let channelwise_lin1 = iris_nn::linear(inp, embed_dim, vb.pp("channelwise.0"))?;
        let channelwise_lin2 = iris_nn::linear(embed_dim, inp, vb.pp("channelwise.2"))?;
...
impl Module for MixingResidualBlock {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        let x_temp = iris_nn::ops::replication_pad2d(&x_temp, 1)?;
...
#[derive(Debug)]
pub struct PaellaVQ {
    in_block_conv: iris_nn::Conv2d,
    out_block_conv: iris_nn::Conv2d,
    down_blocks: Vec<(Option<iris_nn::Conv2d>, MixingResidualBlock)>,
    down_blocks_conv: iris_nn::Conv2d,
    down_blocks_bn: iris_nn::BatchNorm,
    up_blocks_conv: iris_nn::Conv2d,
    up_blocks: Vec<(Vec<MixingResidualBlock>, Option<iris_nn::ConvTranspose2d>)>,
}
...
impl PaellaVQ {
    pub fn new(vb: VarBuilder) -> Result<Self> {
        ...
        let in_block_conv = iris_nn::conv2d(
            IN_CHANNELS * 4,
            C_LEVELS[0],
            1,
            Default::default(),
        ...
        let out_block_conv = iris_nn::conv2d(
            C_LEVELS[0],
            OUT_CHANNELS * 4,
            1,
            Default::default(),
        ...
        for (i, &c_level) in C_LEVELS.iter().enumerate() {
            let conv_block = if i > 0 {
                ...
                let block = iris_nn::conv2d(C_LEVELS[i - 1], c_level, 4, cfg, vb_d.pp(d_idx))?;
                d_idx += 1;
                Some(block)
            } else {
                None
        ...
        let down_blocks_conv = iris_nn::conv2d_no_bias(
            C_LEVELS[1],
            LATENT_CHANNELS,
            1,
            Default::default(),
        ...
        let down_blocks_bn = iris_nn::batch_norm(LATENT_CHANNELS, 1e-5, vb_d.pp(1))?;

        let mut up_blocks = Vec::new();
        let vb_u = vb.pp("up_blocks");
        let mut u_idx = 0;
        let up_blocks_conv = iris_nn::conv2d(
            LATENT_CHANNELS,
            C_LEVELS[1],
            1,
            Default::default(),
        ...
        for (i, &c_level) in C_LEVELS.iter().rev().enumerate() {
            ...
            let conv_block = if i < C_LEVELS.len() - 1 {
                let cfg = iris_nn::ConvTranspose2dConfig {
                    padding: 1,
                    stride: 2,
                    ..Default::default()
                };
                let block = iris_nn::conv_transpose2d(
                    c_level,
    ...
    pub fn encode(&self, xs: &Tensor) -> Result<Tensor> {
        let mut xs = iris_nn::ops::pixel_unshuffle(xs, 2)?.apply(&self.in_block_conv)?;
    ...
    pub fn decode(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        xs.apply(&self.out_block_conv)?
            .apply(&|xs: &_| iris_nn::ops::pixel_shuffle(xs, 2))
    }
}
