use iris_core::{DType, IndexOp, Result, Tensor};
use iris_nn::{layer_norm, LayerNorm, Module, VarBuilder};
...
impl PatchEmbed {
    fn new(
        ...
        let proj = iris_nn::conv2d(in_chans, embed_dim, k_size, cfg, vb.pp("proj"))?;
...
impl iris_core::CustomOp3 for Add3 {
    ...
    fn cpu_fwd(
        &self,
        s1: &iris_core::CpuStorage,
        l1: &iris_core::Layout,
        s2: &iris_core::CpuStorage,
        l2: &iris_core::Layout,
        s3: &iris_core::CpuStorage,
        l3: &iris_core::Layout,
    ) -> Result<(iris_core::CpuStorage, iris_core::Shape)> {
        use rayon::prelude::*;

        let Add3(b, q_h, q_w, k_h, k_w) = *self;
        let s1 = s1.as_slice::<f32>()?;
        let s1 = match l1.contiguous_offsets() {
            None => iris_core::bail!("input1 has to be contiguous"),
            Some((o1, o2)) => &s1[o1..o2],
        };
        let s2 = s2.as_slice::<f32>()?;
        let s2 = match l2.contiguous_offsets() {
            None => iris_core::bail!("input2 has to be contiguous"),
            Some((o1, o2)) => &s2[o1..o2],
        };
        let s3 = s3.as_slice::<f32>()?;
        let s3 = match l3.contiguous_offsets() {
            None => iris_core::bail!("input3 has to be contiguous"),
            Some((o1, o2)) => &s3[o1..o2],
        };
        ...
        let dst = iris_core::WithDType::to_cpu_storage_owned(dst);
...
impl Module for Attention {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        let attn = {
            ...
            iris_nn::ops::softmax_last_dim(&attn)?
        };
        let attn = {
            let _enter = self.span_matmul.enter();
            attn.matmul(&v)?
...
impl Block {
    fn new(
        ...
        let mlp = super::MlpBlock::new(dim, dim * 4, iris_nn::Activation::Gelu, vb.pp("mlp"))?;
...
pub struct ImageEncoderViT {
    ...
    blocks: Vec<Block>,
    neck_conv1: iris_nn::Conv2d,
    neck_ln1: super::LayerNorm2d,
    neck_conv2: iris_nn::Conv2d,
...
impl ImageEncoderViT {
    ...
    pub fn new(
        ...
        let neck_conv1 = iris_nn::conv2d_no_bias(
            embed_dim,
            out_chans,
            1,
            Default::default(),
        ...
        let neck_conv2 = iris_nn::conv2d_no_bias(out_chans, out_chans, 3, cfg, vb.pp("neck.2"))?;
