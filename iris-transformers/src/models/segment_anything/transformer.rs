...
impl Attention {
    fn new(
        ...
        let q_proj = iris_nn::linear(embedding_dim, internal_dim, vb.pp("q_proj"))?;
        let k_proj = iris_nn::linear(embedding_dim, internal_dim, vb.pp("k_proj"))?;
        let v_proj = iris_nn::linear(embedding_dim, internal_dim, vb.pp("v_proj"))?;
        let out_proj = iris_nn::linear(internal_dim, embedding_dim, vb.pp("out_proj"))?;
        Ok(Self {
            q_proj,
            k_proj,
            v_proj,
