use iris_core::{IndexOp, Result, Tensor};
use iris_nn::{Module, VarBuilder};
...
pub struct MaskDecoder {
    iou_token: iris_nn::Embedding,
    mask_tokens: iris_nn::Embedding,
    iou_prediction_head: MlpMaskDecoder,
    output_upscaling_conv1: iris_nn::ConvTranspose2d,
    output_upscaling_ln: super::LayerNorm2d,
    output_upscaling_conv2: iris_nn::ConvTranspose2d,
...
impl MaskDecoder {
    pub fn new(
        ...
        let iou_token = iris_nn::embedding(1, transformer_dim, vb.pp("iou_token"))?;
        let mask_tokens =
            iris_nn::embedding(num_mask_tokens, transformer_dim, vb.pp("mask_tokens"))?;
        let cfg = iris_nn::ConvTranspose2dConfig {
            stride: 2,
            ..Default::default()
        };
        let output_upscaling_conv1 = iris_nn::conv_transpose2d(
            transformer_dim,
            transformer_dim / 4,
            2,
            cfg,
        ...
        let output_upscaling_conv2 = iris_nn::conv_transpose2d(
            transformer_dim / 4,
            transformer_dim / 8,
            2,
            cfg,
