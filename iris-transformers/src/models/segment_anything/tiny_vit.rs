...
use iris_core::{IndexOp, Result, Tensor, D};
use iris_nn::{Conv2dConfig, Module, VarBuilder};
...
struct Conv2dBN {
    c: iris_nn::Conv2d,
    bn: iris_nn::BatchNorm,
    span: tracing::Span,
}

impl Conv2dBN {
    fn new(in_: usize, out: usize, ks: usize, cfg: Conv2dConfig, vb: VarBuilder) -> Result<Self> {
        let c = iris_nn::conv2d_no_bias(in_, out, ks, cfg, vb.pp("c"))?;
        let bn = iris_nn::batch_norm(out, 1e-5, vb.pp("bn"))?;
...
impl Mlp {
    fn new(in_: usize, hidden: usize, vb: VarBuilder) -> Result<Self> {
        let norm = iris_nn::layer_norm(in_, 1e-5, vb.pp("norm"))?;
...
impl Attention {
    fn new(
        ...
        let h = dh + nh_kd * 2;
        let norm = iris_nn::layer_norm(dim, 1e-5, vb.pp("norm"))?;
...
impl TinyViT {
    pub fn new(
        ...
        // let norm_head = iris_nn::layer_norm(last_embed_dim, 1e-5, vb.pp("norm_head"))?;
        // let head = iris_nn::linear(last_embed_dim, num_classes, vb.pp("head"))?;
        let neck_conv1 =
            iris_nn::conv2d_no_bias(last_embed_dim, 256, 1, Default::default(), vb.pp("neck.0"))?;
        let neck_ln1 = super::LayerNorm2d::new(256, 1e-6, vb.pp("neck.1"))?;
        let cfg = iris_nn::Conv2dConfig {
            padding: 1,
            ..Default::default()
        };
        let neck_conv2 = iris_nn::conv2d_no_bias(256, 256, 3, cfg, vb.pp("neck.2"))?;
