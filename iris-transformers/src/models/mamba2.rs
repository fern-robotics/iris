...
impl Mamba2Block {
    pub fn new(layer_idx: usize, cfg: &Config, vb: VarBuilder) -> Result<Self> {
        ...
        let norm = iris_nn::rms_norm(d_inner, 1e-5, vb.pp("norm"))?;
    ...
    pub fn forward(&self, xs: &Tensor, state: &mut State) -> Result<Tensor> {
        ...
        let xbc_conv = iris_nn::ops::silu(&xbc_conv)?;
...
impl Model {
    pub fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        let embedding = iris_nn::embedding(cfg.vocab_size(), cfg.d_model, vb.pp("embeddings"))?;
