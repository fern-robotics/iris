...
use iris_nn::{embedding, rms_norm, Activation, Embedding, Linear, Module, RmsNorm, VarBuilder};
...
impl CustomOp1 for NonZero {
    ...
    fn cpu_fwd(&self, storage: &CpuStorage, layout: &Layout) -> Result<(CpuStorage, Shape)> {
        ...
        let result = match storage {
            iris_core::CpuStorage::U8(vs) => self.nonzero(vs, layout),
            iris_core::CpuStorage::U32(vs) => self.nonzero(vs, layout),
            iris_core::CpuStorage::I16(vs) => self.nonzero(vs, layout),
            iris_core::CpuStorage::I32(vs) => self.nonzero(vs, layout),
            iris_core::CpuStorage::I64(vs) => self.nonzero(vs, layout),
            iris_core::CpuStorage::BF16(vs) => self.nonzero(vs, layout),
            iris_core::CpuStorage::F16(vs) => self.nonzero(vs, layout),
            iris_core::CpuStorage::F32(vs) => self.nonzero(vs, layout),
            iris_core::CpuStorage::F64(vs) => self.nonzero(vs, layout),
            iris_core::CpuStorage::F8E4M3(vs) => self.nonzero(vs, layout),
            // Dummy types don't support nonzero operation
            iris_core::CpuStorage::F6E2M3(_) => {
                return Err(
                    iris_core::Error::UnsupportedDTypeForOp(iris_core::DType::F6E2M3, "nonzero").bt(),
                )
            }
            iris_core::CpuStorage::F6E3M2(_) => {
                return Err(
                    iris_core::Error::UnsupportedDTypeForOp(iris_core::DType::F6E3M2, "nonzero").bt(),
                )
            }
            iris_core::CpuStorage::F4(_) => {
                return Err(iris_core::Error::UnsupportedDTypeForOp(iris_core::DType::F4, "nonzero").bt())
            }
            iris_core::CpuStorage::F8E8M0(_) => {
                return Err(
                    iris_core::Error::UnsupportedDTypeForOp(iris_core::DType::F8E8M0, "nonzero").bt(),
                )
            }
        };
...
impl NonZeroOp for Tensor {
    fn nonzero(&self) -> Result<Tensor> {
        if !self.is_contiguous() {
            return Err(iris_core::Error::RequiresContiguous { op: "nonzero" });
        }
        let original_device = self.device();
        self.to_device(&iris_core::Device::Cpu)?
...
impl DeepSeekV2RotaryEmbedding {
    ...
    pub fn new(cfg: &DeepSeekV2RopeConfig, dtype: DType, dev: &Device) -> Result<Self> {
        match &cfg.rope_scaling {
            Some(DeepSeekV2RopeScaling::LinearOrDynamic {
                scaling_type: _,
                factor: _,
            }) => iris_core::bail!("linear and dynamic rope are not implemented yet!"),
    ...
    pub fn forward(
        ...
        let q_embed = iris_nn::rotary_emb::rope_i(&q.contiguous()?, &cos, &sin)?;
        let k_embed = iris_nn::rotary_emb::rope_i(&k.contiguous()?, &cos, &sin)?;
...
impl Attention {
    fn new(
        ...
        let q = match cfg.q_lora_rank {
            ...
            None => QProj::Plain(iris_nn::linear_no_bias(
                cfg.hidden_size,
                cfg.num_attention_heads * q_head_dim,
                vb.pp("q_proj"),
            )?),
        ...
        let kv_a_proj_with_mqa = iris_nn::linear_b(
            cfg.hidden_size,
    ...
    fn forward(
        ...
        let attn_out = {
            ...
            let att = iris_nn::ops::softmax_last_dim(&att)?;
...
impl Mlp {
    fn new(
        ...
        Ok(Self {
            gate: iris_nn::linear_no_bias(hidden_size, intermediate_size, vb.pp("gate_proj"))?,
            up: iris_nn::linear_no_bias(hidden_size, intermediate_size, vb.pp("up_proj"))?,
            down: iris_nn::linear_no_bias(intermediate_size, hidden_size, vb.pp("down_proj"))?,
            act: cfg.hidden_act,
        })
    }
...
impl MoeGate {
    ...
    fn forward(&self, xs: &Tensor) -> Result<(Tensor, Tensor)> {
        ...
        let scores = match self.cfg.scoring_func {
            ScoringFunc::Softmax => iris_nn::ops::softmax_last_dim(&logits)?,
        };
...
impl DeepSeekV2 {
    pub fn new(cfg: &DeepSeekV2Config, vb: VarBuilder) -> Result<Self> {
        ...
        let lm_head = if !cfg.tie_word_embeddings {
            iris_nn::linear_no_bias(cfg.hidden_size, cfg.vocab_size, vb.pp("lm_head"))?
        } else {
            iris_nn::Linear::new(embed_tokens.embeddings().clone(), None)
        };
