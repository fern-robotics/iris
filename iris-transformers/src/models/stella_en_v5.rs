...
use iris_core::{DType, Device, Error, IndexOp, Module, Result, Tensor, D};
use iris_nn::{layer_norm, Activation, LayerNorm, VarBuilder};
use std::sync::Arc;
...
impl Config {
    ...
    pub fn new_1_5_b_v5(embed_dim: EmbedDim) -> Self {
        ...
        Self {
            ...
            activation_fn: iris_nn::Activation::Silu,
            vocab_size: 151646,
            hidden_size: 1536,
        ...
        match self.variant {
            ModelVariant::Large => {
                    ...
                    (&self.post_attention_layernorm, &self.layernorm)
                {
                    ...
                    return Err(iris_core::error::Error::Msg(
...
pub struct Embeddings {
    ...
    embeddings: iris_nn::Embedding,
    // following are specifically for 400M
    token_type_embeddings: Option<iris_nn::Embedding>,
...
impl Embeddings {
    pub fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        let (embeddings, token_type_embeddings, layer_norm, position_ids) = match cfg.variant {
            ModelVariant::Large => (
                iris_nn::embedding(cfg.vocab_size, cfg.hidden_size, vb.pp("embed_tokens"))?,
                None,
                None,
                None,
            ),
            ModelVariant::Small => {
                ...
                let weight = vb.pp("LayerNorm").get_with_hints(
                    cfg.hidden_size,
                    "weight",
                    iris_nn::Init::Const(1.0),
                )?;
                let bias = vb.pp("LayerNorm").get_with_hints(
                    cfg.hidden_size,
                    "bias",
                    iris_nn::Init::Const(0.0),
                )?;
                let dev = bias.device().clone();

                let layer_norm = iris_nn::LayerNorm::new(weight, bias, cfg.norm_eps);
