...
use iris_nn::{kv_cache::ConcatKvCache, Activation, VarBuilder};
use std::sync::Arc;
...
impl Qwen3Attention {
    pub(crate) fn new(
        ...
        if cfg.use_sliding_window {
            iris_core::bail!("sliding window is not supported")
        }
    ...
    fn forward_flash_attn(
        ...
        let causal = l > 1;
        let ctx = iris_flash_attn::flash_attn(&q, &k, &v, scale, causal)?;
...
impl Model {
    pub fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        ...
        let embed_tokens =
            iris_nn::embedding(cfg.vocab_size, cfg.hidden_size, vb.pp("model.embed_tokens"))?;
