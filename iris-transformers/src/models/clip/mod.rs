...
pub struct ClipModel {
    ...
    vision_model: ClipVisionTransformer,
    visual_projection: iris_nn::Linear,
    text_projection: iris_nn::Linear,
    logit_scale: Tensor,
}
...
impl ClipModel {
    pub fn new(vs: iris_nn::VarBuilder, c: &ClipConfig) -> Result<Self> {
        ...
        let visual_projection = iris_nn::linear_no_bias(
            c.vision_config.embed_dim,
            c.vision_config.projection_dim,
            vs.pp("visual_projection"),
        )?;
        let text_projection = iris_nn::linear_no_bias(
