...
use iris_core::{DType, Device, IndexOp, Result, Tensor, D};
use iris_nn as nn;
use iris_nn::Module;
...
struct ClipTextEmbeddings {
    token_embedding: iris_nn::Embedding,
    position_embedding: iris_nn::Embedding,
    position_ids: Tensor,
}

impl ClipTextEmbeddings {
    fn new(vs: iris_nn::VarBuilder, c: &ClipTextConfig) -> Result<Self> {
        let token_embedding =
            iris_nn::embedding(c.vocab_size, c.embed_dim, vs.pp("token_embedding"))?;
        let position_embedding: nn::Embedding = iris_nn::embedding(
...
struct ClipAttention {
    k_proj: iris_nn::Linear,
    v_proj: iris_nn::Linear,
    q_proj: iris_nn::Linear,
    out_proj: iris_nn::Linear,
    head_dim: usize,
    scale: f64,
...
impl ClipAttention {
    fn new(vs: iris_nn::VarBuilder, c: &EncoderConfig) -> Result<Self> {
        let embed_dim = c.embed_dim();
        let num_attention_heads = c.num_attention_heads();
        let k_proj = iris_nn::linear(embed_dim, embed_dim, vs.pp("k_proj"))?;
        let v_proj = iris_nn::linear(embed_dim, embed_dim, vs.pp("v_proj"))?;
        let q_proj = iris_nn::linear(embed_dim, embed_dim, vs.pp("q_proj"))?;
        let out_proj = iris_nn::linear(embed_dim, embed_dim, vs.pp("out_proj"))?;
    ...
    fn forward(&self, xs: &Tensor, causal_attention_mask: Option<&Tensor>) -> Result<Tensor> {
        ...
        let attn_weights = iris_nn::ops::softmax(&attn_weights, D::Minus1)?;
...
impl ClipMlp {
    fn new(vs: iris_nn::VarBuilder, c: &EncoderConfig) -> Result<Self> {
        let fc1 = iris_nn::linear(c.embed_dim(), c.intermediate_size(), vs.pp("fc1"))?;
        let fc2 = iris_nn::linear(c.intermediate_size(), c.embed_dim(), vs.pp("fc2"))?;
...
struct ClipEncoderLayer {
    ...
    layer_norm1: iris_nn::LayerNorm,
    mlp: ClipMlp,
    layer_norm2: iris_nn::LayerNorm,
}

impl ClipEncoderLayer {
    fn new(vs: iris_nn::VarBuilder, c: &EncoderConfig) -> Result<Self> {
        let self_attn = ClipAttention::new(vs.pp("self_attn"), c)?;
        let layer_norm1 = iris_nn::layer_norm(c.embed_dim(), 1e-5, vs.pp("layer_norm1"))?;
        let mlp = ClipMlp::new(vs.pp("mlp"), c)?;
        let layer_norm2 = iris_nn::layer_norm(c.embed_dim(), 1e-5, vs.pp("layer_norm2"))?;
...
impl ClipEncoder {
    pub fn new(vs: iris_nn::VarBuilder, c: &EncoderConfig) -> Result<Self> {
        let vs = vs.pp("layers");
...
pub struct ClipTextTransformer {
    ...
    encoder: ClipEncoder,
    final_layer_norm: iris_nn::LayerNorm,
}

impl ClipTextTransformer {
    pub fn new(vs: iris_nn::VarBuilder, c: &ClipTextConfig) -> Result<Self> {
        ...
        let final_layer_norm = iris_nn::layer_norm(c.embed_dim, 1e-5, vs.pp("final_layer_norm"))?;
