...
use iris_core::{Context, IndexOp, Result, Shape, Tensor, D};
use iris_nn as nn;
use iris_nn::Module;
use nn::Conv2dConfig;
...
impl ClipVisionEmbeddings {
    fn new(vs: iris_nn::VarBuilder, c: &ClipVisionConfig) -> Result<Self> {
        ...
        let position_embedding =
            iris_nn::embedding(num_positions, c.embed_dim, vs.pp("position_embedding"))?;
        let patch_embedding = iris_nn::conv2d_no_bias(
            c.num_channels,
            c.embed_dim,
            c.patch_size,
            conv2dconfig,
...
pub struct ClipVisionTransformer {
    ...
    encoder: ClipEncoder,
    pre_layer_norm: iris_nn::LayerNorm,
    final_layer_norm: iris_nn::LayerNorm,
}

impl ClipVisionTransformer {
    pub fn new(vs: iris_nn::VarBuilder, c: &ClipVisionConfig) -> Result<Self> {
        let embeddings = ClipVisionEmbeddings::new(vs.pp("embeddings"), c)?;
        let pre_layer_norm = iris_nn::layer_norm(c.embed_dim, 1e-5, vs.pp("pre_layrnorm"))?;
        let encoder = ClipEncoder::new(vs.pp("encoder"), &EncoderConfig::Vision(c.clone()))?;
        let final_layer_norm = iris_nn::layer_norm(c.embed_dim, 1e-5, vs.pp("post_layernorm"))?;
        Ok(Self {
            embeddings,
            encoder,
            final_layer_norm,
