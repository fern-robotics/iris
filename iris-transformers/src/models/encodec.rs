...
use iris_core::{DType, IndexOp, Layout, Module, Result, Shape, Tensor, D};
use iris_nn::{conv1d, Conv1d, ConvTranspose1d, VarBuilder};
...
fn pad1d(xs: &Tensor, pad_l: usize, pad_r: usize, mode: PadMode) -> Result<Tensor> {
    match mode {
        ...
        PadMode::Reflect => iris_core::bail!("pad-mode 'reflect' is not supported"),
...
pub fn conv_transpose1d_weight_norm(
    ...
    kernel_size: usize,
    bias: bool,
    config: iris_nn::ConvTranspose1dConfig,
    vb: VarBuilder,
...
impl iris_core::CustomOp2 for CodebookEncode {
    ...
    fn cpu_fwd(
        &self,
        lhs_storage: &iris_core::CpuStorage,
        lhs_layout: &Layout,
        rhs_storage: &iris_core::CpuStorage,
        rhs_layout: &Layout,
    ) -> Result<(iris_core::CpuStorage, Shape)> {
        use rayon::prelude::*;

        let (lhs_dim1, lhs_dim2) = lhs_layout.shape().dims2()?;
        let (rhs_dim1, rhs_dim2) = rhs_layout.shape().dims2()?;
        if lhs_dim2 != rhs_dim2 {
            iris_core::bail!("CodebookEncode, mismatch on last dim, {lhs_layout:?} {rhs_layout:?}");
        }
        if lhs_dim2 == 0 {
            iris_core::bail!("CodebookEncode, empty last dim {lhs_layout:?}")
        }
        let lhs = match lhs_layout.contiguous_offsets() {
            None => iris_core::bail!("CodebookEncode, lhs has to be contiguous, got {lhs_layout:?}"),
            Some((o1, o2)) => {
        ...
        let rhs = match rhs_layout.contiguous_offsets() {
            None => iris_core::bail!("CodebookEncode, rhs has to be contiguous, got {rhs_layout:?}"),
            Some((o1, o2)) => {
...
impl EuclideanCodebook {
    pub fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        ...
        Ok(Self {
            inited,
            cluster_size,
            embed: iris_nn::Embedding::new(embed, cfg.codebook_dim()),
            embed_avg,
            c2,
        })
    }
...
impl EncodecLSTM {
    pub fn new(dim: usize, cfg: &Config, vb: VarBuilder) -> Result<Self> {
        ...
        for layer_idx in 0..cfg.num_lstm_layers {
            let config = iris_nn::LSTMConfig {
                layer_idx,
                ..Default::default()
            };
            let lstm = iris_nn::lstm(dim, dim, config, vb.clone())?;
            layers.push(lstm)
        }
        Ok(Self { layers })
    }
...
impl EncodecConvTranspose1d {
    fn new(
        ...
        let cfg = iris_nn::ConvTranspose1dConfig {
            stride,
            ..Default::default()
        };
    ...
    pub fn new(
        ...
        let norm = match cfg.norm_type {
            ...
            NormType::TimeGroupNorm => {
                let gn = iris_nn::group_norm(1, out_c, 1e-5, vb.pp("norm"))?;
                Some(gn)
            }
        };
        Ok(Self {
