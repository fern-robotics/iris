...
use iris_nn::{embedding, linear_b as linear, Embedding, LayerNorm, Linear, Module, VarBuilder};
...
impl Config {
    pub fn validate(&self) -> Result<()> {
        if self.alibi {
            iris_core::bail!("alibi is not supported");
        }
        if self.new_decoder_architecture {
            iris_core::bail!("new_decoder_architecture is not supported");
        }
        if self.n_head_kv.is_some() {
            iris_core::bail!("n_head_kv is not supported");
        }
        Ok(())
    }
...
impl FalconAttention {
    ...
    fn forward(&mut self, x: &Tensor, mask: Option<&Tensor>, past_kv_len: usize) -> Result<Tensor> {
        ...
        let attention_scores =
            iris_nn::ops::softmax(&attention_scores.to_dtype(DType::F32)?, D::Minus1)?
                .to_dtype(x.dtype())?;
