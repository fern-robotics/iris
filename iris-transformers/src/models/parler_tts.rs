...
use iris_nn::{layer_norm, linear_b as linear, Activation, LayerNorm, Linear, VarBuilder};
...
impl Attention {
    fn new(
        ...
        if cfg.rope_embeddings {
            iris_core::bail!("rope embeddings are not supported");
        }
...
#[derive(Debug, Clone)]
pub struct Decoder {
    embed_tokens: Vec<iris_nn::Embedding>,
...
impl Decoder {
    pub fn new(cfg: &DecoderConfig, vb: VarBuilder) -> Result<Self> {
        ...
        for embed_idx in 0..cfg.num_codebooks {
            let e = iris_nn::embedding(cfg.vocab_size + 1, cfg.hidden_size, vb_e.pp(embed_idx))?;
            embed_tokens.push(e)
        }
    ...
    pub fn forward(
        ...
        if num_codebooks != self.num_codebooks {
            iris_core::bail!("unexpected num codebooks in input {:?}", input_ids.shape())
        }
...
#[derive(Debug, Clone)]
pub struct Model {
    pub embed_prompts: iris_nn::Embedding,
...
impl Model {
    pub fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        ...
        let embed_prompts = iris_nn::embedding(
            cfg.vocab_size,
    ...
    pub fn generate(
        ...
        let all_audio_tokens = Tensor::new(all_audio_tokens, &iris_core::Device::Cpu)?;
        Ok(all_audio_tokens)
    }
