...
use iris_core::{Context, Module, ModuleT, Result, Tensor, D};
use iris_nn::{conv2d_no_bias, layer_norm, Activation, Conv2dConfig, VarBuilder};
...
impl SegformerOverlapPatchEmbeddings {
    fn new(
        ...
        let layer_norm =
            iris_nn::layer_norm(hidden_size, config.layer_norm_eps, vb.pp("layer_norm"))?;
        Ok(Self {
            projection,
            layer_norm,
        })
...
impl Module for SegformerEfficientSelfAttention {
    fn forward(&self, x: &Tensor) -> Result<Tensor> {
        ...
        let attention_scores = iris_nn::ops::softmax_last_dim(&attention_scores)?;
...
struct SegformerLayer {
    layer_norm_1: iris_nn::LayerNorm,
    attention: SegformerAttention,
    layer_norm_2: iris_nn::LayerNorm,
    mlp: SegformerMixFFN,
}
...
struct SegformerEncoder {
    ...
    layer_norms: Vec<iris_nn::LayerNorm>,
}
...
struct SegformerDecodeHead {
    ...
    linear_fuse: iris_nn::Conv2d,
    batch_norm: iris_nn::BatchNorm,
    classifier: iris_nn::Conv2d,
}
...
impl SegformerDecodeHead {
    fn new(config: &Config, num_labels: usize, vb: VarBuilder) -> Result<Self> {
        ...
        let batch_norm = iris_nn::batch_norm(
            config.decoder_hidden_size,
            config.layer_norm_eps,
            vb.pp("batch_norm"),
        )?;
