...
use iris_nn::{Embedding, Module, VarBuilder};
...
impl Embeddings {
    fn load(vb: VarBuilder, config: &Config) -> Result<Self> {
        let word_embeddings =
            iris_nn::embedding(config.vocab_size, config.dim, vb.pp("word_embeddings"))?;
        let position_embeddings = iris_nn::embedding(
...
impl MultiHeadSelfAttention {
    fn forward(&self, hidden_states: &Tensor, attention_mask: &Tensor) -> Result<Tensor> {
        ...
        let weights = iris_nn::ops::softmax(&scores, iris_core::D::Minus1)?;
