...
use iris_core::{DType, Device, Module, Result, Storage, Tensor};
use iris_nn::attention::cpu_flash::causal::causal_decode_f32_interleaved;
use iris_nn::attention::{flash_attn, AttnMask};
use iris_nn::kv_cache::{rope_i_inplace, InterleavedKvCache, KvCache, RawInterleavedKvCache};
use iris_nn::Activation;
use std::io::Write;
use std::sync::Arc;

const MAX_SEQ_LEN: usize = 4096;
use iris_core::IndexOp;
    ...
    pub fn from_gguf(ct: &gguf_file::Content) -> Result<Self> {
        ...
        let get_u32 = |key: &str| -> Result<usize> {
            metadata
                ...
                .map(|v| v as usize)
                .ok_or_else(|| {
                    iris_core::Error::Msg(format!("Missing or invalid metadata key: {}", key))
                })
        };
...
impl RmsNorm {
    ...
    fn forward(&self, x: &Tensor) -> Result<Tensor> {
        ...
        let hidden_size = x.dim(iris_core::D::Minus1)?;
        let x = x.to_dtype(internal_dtype)?;
        let norm_x = (x.sqr()?.sum_keepdim(iris_core::D::Minus1)? / hidden_size as f64)?;
...
impl RotaryEmbedding {
    ...
    pub fn apply_rotary_emb(
        ...
        let (q_embed, k_embed) = if self.use_interleaved {
            ...
            let q_embed = iris_nn::rotary_emb::rope(&q.contiguous()?, &cos, &sin)?;
            let k_embed = iris_nn::rotary_emb::rope(&k.contiguous()?, &cos, &sin)?;
            (q_embed, k_embed)
        };
        Ok((q_embed, k_embed))
    }
...
impl QuantizedAttention {
    fn new(
        ...
        let (q_proj, k_proj) = if use_flash_attn {
            ...
            use iris_core::quantized::{GgmlDType, QTensor};
    ...
    fn forward(&mut self, x: &Tensor, mask: Option<&Tensor>, offset: usize) -> Result<Tensor> {
            ...
            && x.dtype() == DType::F32
        {
            ...
            let q_flat: &[f32] = match &*q_g {
                ...
                _ => iris_core::bail!("Expected CPU storage"),
            };
...
impl QuantizedModelForCausalLM {
    pub fn from_gguf<P: AsRef<std::path::Path>>(
        ...
        let embed_tokens = iris_nn::Embedding::new(embed_tensor_gpu, config.hidden_size);
