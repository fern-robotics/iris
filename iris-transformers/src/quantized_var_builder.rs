...
use iris_core::{Device, Result, Shape};
use std::sync::Arc;
...
impl VarBuilder {
    pub fn from_gguf<P: AsRef<std::path::Path>>(p: P, device: &Device) -> Result<Self> {
        ...
        let content = iris_core::quantized::gguf_file::Content::read(&mut file)?;
    ...
    pub fn from_gguf_buffer(buffer: &[u8], device: &Device) -> Result<Self> {
        ...
        let content = iris_core::quantized::gguf_file::Content::read(&mut cursor)?;
        let mut data = std::collections::HashMap::new();
        for tensor_name in content.tensor_infos.keys() {
    ...
    pub fn get<S: Into<Shape>>(&self, s: S, name: &str) -> Result<Arc<QTensor>> {
        ...
        match self.data.get(&path) {
            None => {
                iris_core::bail!("cannot find tensor {path}")
            }
            Some(qtensor) => {
                let shape = s.into();
    ...
    pub fn get_no_shape(&self, name: &str) -> Result<Arc<QTensor>> {
        ...
        match self.data.get(&path) {
            None => {
                iris_core::bail!("cannot find tensor {name}")
            }
