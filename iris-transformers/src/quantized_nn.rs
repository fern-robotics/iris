...
impl Embedding {
    pub fn new(d1: usize, d2: usize, vb: VarBuilder) -> Result<Self> {
        ...
        let inner = iris_nn::Embedding::new(embeddings, d2);
...
impl Module for Linear {
    fn forward(&self, x: &Tensor) -> iris_core::Result<Tensor> {
...
pub fn layer_norm(size: usize, eps: f64, vb: VarBuilder) -> Result<iris_nn::LayerNorm> {
    let weight = vb.get(size, "weight")?.dequantize(vb.device())?;
    let bias = vb.get(size, "bias")?.dequantize(vb.device())?;
    Ok(iris_nn::LayerNorm::new(weight, bias, eps))
}

pub fn layer_norm_no_bias(size: usize, eps: f64, vb: VarBuilder) -> Result<iris_nn::LayerNorm> {
    let weight = vb.get(size, "weight")?.dequantize(vb.device())?;
    Ok(iris_nn::LayerNorm::new_no_bias(weight, eps))
}
...
impl Module for RmsNorm {
    fn forward(&self, x: &Tensor) -> Result<Tensor> {
        let _enter = self.span.enter();
        iris_nn::ops::rms_norm(x, &self.weight, self.eps as f32)
    }
}
