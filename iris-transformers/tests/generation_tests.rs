...
fn sample_with_top_k() -> Result<()> {
    let mut logits_process = LogitsProcessor::from_sampling(
        42,
        iris_transformers::generation::Sampling::TopK {
            k: 1,
            temperature: 1.0,
        },
    );
...
fn sample_gumbel() -> Result<()> {
    let mut logits_process = LogitsProcessor::from_sampling(
        42,
        iris_transformers::generation::Sampling::GumbelSoftmax { temperature: 1.0 },
    );
    let logits = Tensor::new(&[-1.0, 0.0, 0.2, 1.0], &Device::Cpu)?;
    let sm = iris_nn::ops::softmax(&logits, 0)?.to_vec1::<f64>()?;
