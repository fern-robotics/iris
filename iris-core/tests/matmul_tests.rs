use iris_core::{test_device, DType, Device, IndexOp, Result, Tensor};
...
fn tensor_dot() -> Result<()> {
    ...
    iris_core::test_utils::assert_tensor_eq(&dot_ret, &expected)?;
    Ok(())
}
...
fn tensor_mv() -> Result<()> {
    ...
    iris_core::test_utils::assert_tensor_eq(&mv_ret, &expected)?;
    Ok(())
}
