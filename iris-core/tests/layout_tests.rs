use iris_core::{test_device, Device, IndexOp, Result, Tensor};
use iris_core as candle;
...
fn strided_blocks() -> Result<()> {
    ...
    let tensor = tensor.i((.., 1))?;
    match tensor.strided_blocks() {
        iris_core::StridedBlocks::UniformBlocks {
            start_offset,
            block_len,
            count,
            src_stride,
    ...
    match tensor.t()?.strided_blocks() {
        iris_core::StridedBlocks::MultipleBlocks {
            block_start_index,
            block_len,
        } => {
