...
#[test]
fn test_pth() {
    let tensors = iris_core::pickle::PthTensors::new("tests/test.pt", None).unwrap();
...
fn test_pth_with_key() {
    let tensors =
        iris_core::pickle::PthTensors::new("tests/test_with_key.pt", Some("model_state_dict"))
            .unwrap();
    tensors.get("test").unwrap().unwrap();
}
...
fn test_pth_fortran_contiguous() {
    let tensors =
        iris_core::pickle::PthTensors::new("tests/fortran_tensor_3d.pth", None).unwrap();
