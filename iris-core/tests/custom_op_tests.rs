...
use iris_core::cpu_backend;
use iris_core::test_utils::to_vec1_round;
use iris_core::{CpuStorage, CustomOp1, DType, Device, Error, Layout, Result, Shape, Tensor};
...
impl CustomOp1 for Elu {
    ...
    fn cpu_fwd(&self, s: &CpuStorage, l: &Layout) -> Result<(CpuStorage, Shape)> {
        let storage = iris_core::map_dtype!(
            "elu",
            s,
...
fn custom_op1_with_backward() -> Result<()> {
    let cpu = &Device::Cpu;
    let t = iris_core::Var::new(&[-2f32, 0f32, 2f32], cpu)?;
...
impl iris_core::InplaceOp1 for Elu {
    fn name(&self) -> &'static str {
        "elu"
    }
    ...
    fn cpu_fwd(&self, s: &mut CpuStorage, _l: &Layout) -> Result<()> {
        ...
        match s {
            ...
            _ => iris_core::bail!("unsupported dtype for inplace elu"),
        }
        Ok(())
    }
}
...
fn ug_op() -> Result<()> {
    let kernel = {
        ...
        let layout = iris_ug::Layout::from_shape(&[12]);
        let ptr = op::Arg::ptr(iris_ug::DType::F32);
        let src = op::load(ptr.id(), layout.clone(), iris_ug::DType::F32)?;
        ...
        let opts: iris_ug::lower_op::Opts = Default::default();
        kernel.lower(&opts)?
    };
    let device = if iris_core::utils::cuda_is_available() {
        Device::new_cuda(0)?
    } else if iris_core::utils::metal_is_available() {
        Device::new_metal(0)?
    } else {
        iris_core::bail!("metal/cuda is mandatory for this test")
    };
    let op = iris_core::UgIOp1::new("test", kernel, &device)?;
