...
fn test_flip_backprop() -> Result<()> {
    ...
    iris_core::test_utils::assert_tensor_eq(&y, &expected_y)?;

    let z = y.flip(&[1])?;
    let expected_z = Tensor::from_vec(vec![6.0, 4.0, 6.0, 4.0], (2, 2), device)?;
    iris_core::test_utils::assert_tensor_eq(&z, &expected_z)?;
    ...
    iris_core::test_utils::assert_tensor_eq(grad_x, &expected_grad)?;
