use iris_core::quantized::{gguf_file, GgmlDType, QTensor};
...
fn run_print(
    ...
    if full {
        iris_core::display::set_print_options_full();
    }
    if let Some(line_width) = line_width {
        iris_core::display::set_line_width(line_width)
    }
    ...
    match format {
        Format::Npz => {
            let tensors = iris_core::npy::NpzTensors::new(file)?;
        ...
        Format::Safetensors => {
            use iris_core::safetensors::Load;
            let tensors = unsafe { iris_core::safetensors::MmapedSafetensors::new(file)? };
        ...
        Format::Pth => {
            let pth_file = iris_core::pickle::PthTensors::new(file, None)?;
            let names = if names.is_empty() {
                pth_file
                    .tensor_infos()
                    .keys()
        ...
        Format::Pickle => {
            iris_core::bail!("pickle format is not supported for print")
        }
        Format::Ggml => {
            let mut file = std::fs::File::open(file)?;
            let content = iris_core::quantized::ggml_file::Content::read(&mut file, device)?;
            let names = if names.is_empty() {
    ...
    match format {
        ...
        Format::Safetensors => {
            ...
            for (name, view) in tensors.iter() {
                ...
                let dtype = match iris_core::DType::try_from(dtype) {
        ...
        Format::Pth => {
            let mut tensors = iris_core::pickle::read_pth_tensor_info(file, verbose, None)?;
        ...
        Format::Pickle => {
            ...
            let mut stack = iris_core::pickle::Stack::empty();
...
fn run_quantize_safetensors(
    ...
    for in_file in in_files.iter() {
        let in_tensors = iris_core::safetensors::load(in_file, &Device::Cpu)?;
...
fn run_dequantize(
    ...
    iris_core::safetensors::save(&tensors, out_file)?;
    Ok(())
}
...
fn run_quantize(
    ...
    if in_files.is_empty() {
        iris_core::bail!("no specified input files")
    }
    if let Some(extension) = out_file.extension() {
        if extension == "safetensors" {
            iris_core::bail!("the generated file cannot use the safetensors extension")
        }
    }
    ...
    if in_files.len() != 1 {
        iris_core::bail!("only a single in-file can be used when quantizing gguf files")
    }
