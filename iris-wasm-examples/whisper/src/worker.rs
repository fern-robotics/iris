...
use iris_core::{safetensors::Load, DType, Device, IndexOp, Tensor, D};
use iris_nn::{ops::softmax, VarBuilder};
pub use iris_transformers::models::whisper::{self as m, Config};
...
impl Decoder {
    ...
    pub fn load(md: ModelData) -> anyhow::Result<Self> {
        ...
        let model = if md.quantized {
            let vb = iris_transformers::quantized_var_builder::VarBuilder::from_gguf_buffer(
                &md.weights,
                &device,
            )?;
...
pub fn detect_language(model: &mut Model, tokenizer: &Tokenizer, mel: &Tensor) -> Result<u32, E> {
    ...
    let probs = iris_nn::ops::softmax(&logits, D::Minus1)?;
