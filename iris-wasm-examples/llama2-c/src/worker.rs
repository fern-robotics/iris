...
use iris_core::{DType, Device, IndexOp, Result, Shape, Tensor};
...
impl Model {
    fn run(
        ...
        let mut tokens = self
            ...
            .map_err(|m| iris_core::Error::Msg(m.to_string()))?
            .get_ids()
            .to_vec();
...
impl Model {
    pub fn load(md: ModelData) -> Result<Self> {
        ...
        let tokenizer =
            Tokenizer::from_bytes(&md.tokenizer).map_err(|m| iris_core::Error::Msg(m.to_string()))?;
        Ok(Self {
            cache,
            config,
            llama,
